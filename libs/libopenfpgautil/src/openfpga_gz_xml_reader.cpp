...
static bool load_gz_to_buffer(const std::filesystem::path& path, std::vector<char>& buffer) {
...
pugiutil::loc_data load_xml(pugi::xml_document& doc, std::string_view filename) {
    ...
    // 2. Fallback: Parse as a regular uncompressed XML file if the buffer is empty
    if (xml_data.empty()) {
