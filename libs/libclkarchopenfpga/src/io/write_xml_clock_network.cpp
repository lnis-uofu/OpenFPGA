...
static int write_xml_clock_tree_taps(pugi::xml_node& root_node,
  ...
  pugi::xml_node taps_node = root_node.append_child(XML_CLOCK_TREE_TAPS_NODE_NAME);

  /* Depends on the type */
  for (ClockTapId tap_id : clk_ntwk.tree_taps(tree_id)) {
    switch (clk_ntwk.tap_type(tap_id)) {
      case ClockNetwork::e_tap_type::ALL: {
        pugi::xml_node tap_node = taps_node.append_child(XML_CLOCK_TREE_TAP_ALL_NODE_NAME);
      ...
      case ClockNetwork::e_tap_type::SINGLE: {
        pugi::xml_node tap_node = taps_node.append_child(XML_CLOCK_TREE_TAP_SINGLE_NODE_NAME);
        tap_node.append_attribute(XML_CLOCK_TREE_TAP_ATTRIBUTE_FROM_PIN) =
          clk_ntwk.tap_from_port(tap_id).to_verilog_string().c_str();
        tap_node.append_attribute(XML_CLOCK_TREE_TAP_ATTRIBUTE_TO_PIN) =
                            clk_ntwk.tap_to_port(tap_id).c_str();
      ...
      case ClockNetwork::e_tap_type::REGION: {
        pugi::xml_node tap_node = taps_node.append_child(XML_CLOCK_TREE_TAP_REGION_NODE_NAME);
        tap_node.append_attribute(XML_CLOCK_TREE_TAP_ATTRIBUTE_FROM_PIN) =
          clk_ntwk.tap_from_port(tap_id).to_verilog_string().c_str();
        tap_node.append_attribute(XML_CLOCK_TREE_TAP_ATTRIBUTE_TO_PIN) =
                            clk_ntwk.tap_to_port(tap_id).c_str();
...
static int write_xml_clock_spine_switch_point(
  pugi::xml_node& root_node, const ClockNetwork& clk_ntwk, const ClockSpineId& spine_id,
  const ClockSwitchPointId& switch_point_id) {

  pugi::xml_node sw_node = root_node.append_child(XML_CLOCK_SPINE_SWITCH_POINT_NODE_NAME);
  ...
  sw_node.append_attribute(XML_CLOCK_SPINE_SWITCH_POINT_ATTRIBUTE_X) = std::to_string(coord.x()).c_str();
  sw_node.append_attribute(XML_CLOCK_SPINE_SWITCH_POINT_ATTRIBUTE_Y) = std::to_string(coord.y()).c_str();
  ...
  if (!clk_ntwk.spine_switch_point_internal_drivers(spine_id, switch_point_id)
    ...
    for (ClockInternalDriverId int_driver_id :
         clk_ntwk.spine_switch_point_internal_drivers(spine_id,
                                                      switch_point_id)) {
      pugi::xml_node int_driver_node = sw_node.append_child(XML_CLOCK_SPINE_SWITCH_POINT_INTERNAL_DRIVER_NODE_NAME);
      int_driver_node.append_attribute(XML_CLOCK_SPINE_SWITCH_POINT_INTERNAL_DRIVER_ATTRIBUTE_FROM_PIN) = 
        clk_ntwk.internal_driver_from_pin(int_driver_id).c_str();
      int_driver_node.append_attribute(XML_CLOCK_SPINE_SWITCH_POINT_INTERNAL_DRIVER_ATTRIBUTE_TO_PIN) = 
...
static int write_xml_clock_spine_intermediate_drivers(
  ...
  pugi::xml_node int_driver_node = root_node.append_child(XML_CLOCK_SPINE_INTERMEDIATE_DRIVER_NODE_NAME);
  ...
  for (ClockInternalDriverId int_driver_id : int_drivers) {
    pugi::xml_node tap_node = int_driver_node.append_child(XML_CLOCK_SPINE_INTERMEDIATE_DRIVER_TAP_NODE_NAME);
    tap_node.append_attribute(XML_CLOCK_SPINE_INTERMEDIATE_DRIVER_ATTRIBUTE_FROM_PIN) =
      clk_ntwk.internal_driver_from_pin(int_driver_id).c_str();
    tap_node.append_attribute(XML_CLOCK_SPINE_INTERMEDIATE_DRIVER_ATTRIBUTE_TO_PIN) =
                        clk_ntwk.internal_driver_to_pin(int_driver_id)
                          .to_verilog_string()
                          .c_str();
  }
...
static int write_xml_clock_spine(pugi::xml_node& root_node, const ClockNetwork& clk_ntwk,
  ...
  for (const vtr::Point<int>& coord : clk_ntwk.spine_coordinates(spine_id)) {
    write_xml_clock_spine_intermediate_drivers(spine_node, clk_ntwk, spine_id, coord);
  }

  for (const ClockSwitchPointId& switch_point_id :
       clk_ntwk.spine_switch_points(spine_id)) {
    write_xml_clock_spine_switch_point(spine_node, clk_ntwk, spine_id, switch_point_id);
  }
...
static int write_xml_clock_tree(pugi::xml_node& root_node,
  ...
  tree_node.append_attribute(XML_CLOCK_TREE_ATTRIBUTE_NAME) = clk_ntwk.tree_name(tree_id).c_str();
...
int write_xml_clock_network(const char* fname, const ClockNetwork& clk_ntwk) {
  ...
  root_node.append_child(pugi::node_comment).set_value("This file is automatically generated!!! Do not modify by hand!");
  ...
  root_node.append_attribute(XML_CLOCK_NETWORK_ATTRIBUTE_DEFAULT_DRIVER_SWITCH) =
