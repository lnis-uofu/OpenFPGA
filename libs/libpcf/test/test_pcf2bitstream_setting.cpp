...
int main(int argc, const char** argv) {
  ...
  openfpga::read_pcf(
    argv[1], pcf_data,
    pcf_custom_command, true, true); /*pcf reader will set custom constraints based on
                            custom commands*/
  ...
  int status = pcf2bitstream_setting(pcf_data, bitstream_setting, io_pin_table, true);
  return status;
}
