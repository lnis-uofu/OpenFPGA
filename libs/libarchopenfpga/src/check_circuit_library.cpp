 ...
 ***********************************************************************/
static size_t check_circuit_library_unique_names(
  ...
  for (size_t i = 0; i < circuit_lib.num_models(); ++i) {
    ...
    for (size_t j = i + 1; j < circuit_lib.num_models(); ++j) {
      ...
      VTR_LOG_ERROR(
        "Circuit model(index=%d) and (index=%d) share the same name, which is "
        "invalid!\n",
        i, j, i_name.c_str());
 ...
 *  If not found, we give an error
 ***********************************************************************/
static size_t check_circuit_library_unique_prefix(
  const CircuitLibrary& circuit_lib) {
  size_t num_err = 0;
  ...
  for (size_t i = 0; i < circuit_lib.num_models(); ++i) {
    ...
    for (size_t j = i + 1; j < circuit_lib.num_models(); ++j) {
      ...
      VTR_LOG_ERROR(
        "Circuit model(name=%s) and (name=%s) share the same prefix, which is "
        "invalid!\n",
        circuit_lib.model_name(CircuitModelId(i)).c_str(),
        circuit_lib.model_name(CircuitModelId(j)).c_str(), i_prefix.c_str());
      /* Incremental the counter for errors */
      num_err++;
    }
  }
 ...
 *  If not found, we give an error
 ***********************************************************************/
static size_t check_circuit_model_required(
  const CircuitLibrary& circuit_lib,
  ...
  for (const auto& port_type : port_types_to_check) {
    if (0 == circuit_lib.model_ports_by_type(circuit_model, port_type).size()) {
      VTR_LOG_ERROR("%s circuit model(name=%s) does not have %s port\n",
                    CIRCUIT_MODEL_TYPE_STRING[size_t(
                      circuit_lib.model_type(circuit_model))],
...
size_t check_one_circuit_model_port_size_required(
  ...
  if (port_size_to_check != circuit_lib.port_size(circuit_port)) {
    VTR_LOG_ERROR(
      "Expect circuit model %s to have %d %s ports but only see %d!\n",
      circuit_lib.model_name(circuit_model).c_str(), port_size_to_check,
      CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(
...
size_t check_one_circuit_model_port_type_and_size_required(
  ...
  std::vector<CircuitPortId> ports = circuit_lib.model_ports_by_type(
    circuit_model, port_type_to_check, false == include_global_ports);
  ...
  for (const auto& port : ports) {
    num_err += check_one_circuit_model_port_size_required(
      circuit_lib, circuit_model, port, port_size_to_check);
  }
 ...
 *  If not found, we give an error
 ***********************************************************************/
static size_t check_circuit_model_port_required(
  const CircuitLibrary& circuit_lib,
  const enum e_circuit_model_type& circuit_model_type_to_check,
  const std::vector<enum e_circuit_model_port_type>& port_types_to_check) {
  size_t num_err = 0;
  ...
  for (const auto& id :
       circuit_lib.models_by_type(circuit_model_type_to_check)) {
    num_err += check_one_circuit_model_port_required(circuit_lib, id,
                                                     port_types_to_check);
  }
 ...
 *  to be specified
 ***********************************************************************/
static size_t check_default_circuit_model_by_types(
  const CircuitLibrary& circuit_lib) {
  size_t num_err = 0;

  for (size_t itype = 0; itype < NUM_CIRCUIT_MODEL_TYPES; ++itype) {
    std::vector<CircuitModelId> curr_models =
      circuit_lib.models_by_type(e_circuit_model_type(itype));
    ...
    if (0 == found_default_counter) {
      VTR_LOG_ERROR(
        "Miss a default circuit model for the type %s! Try to define it in "
    ...
    if (1 < found_default_counter) {
      VTR_LOG_ERROR(
        "Found >1 default circuit models for the type %s! Expect only one!\n",
        CIRCUIT_MODEL_TYPE_STRING[itype]);
      num_err++;
    }
  }
 ...
 *  If not found, we give an error
 ***********************************************************************/
static size_t check_required_default_circuit_model(
  const CircuitLibrary& circuit_lib,
  ...
  if (CircuitModelId::INVALID() ==
    ...
    VTR_LOG_ERROR(
      "Miss a default circuit model for the type %s! Try to define it in your "
      "architecture file!\n",
...
size_t check_ff_circuit_model_ports(const CircuitLibrary& circuit_lib,
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_INPUT, 3, 1, false);
  /* Check if we have a clock */
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_CLOCK, 1, 1, false);

  /* Check if we have output */
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_OUTPUT, 1, 1, false);
...
size_t check_ccff_circuit_model_ports(const CircuitLibrary& circuit_lib,
  ...
  /* Check if we have D, Set and Reset */
  /* We can have either 1 input which is D or 2 inputs which are D and scan
   * input */
  size_t num_input_ports =
    circuit_lib
      .model_ports_by_type(circuit_model, CIRCUIT_MODEL_PORT_INPUT, true)
      .size();
  if ((1 != num_input_ports) && (2 != num_input_ports)) {
    VTR_LOG_ERROR(
      "Configuration flip-flop '%s' must have either 1 or 2 %s ports!\n\tAmong "
      "which:\n\t\tthe first input is a regular input (e.g., D)\n\t\tand the "
      "other could be scan-chain input (e.g., SI)\n",
      circuit_lib.model_name(circuit_model).c_str(),
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_INPUT, num_input_ports, 1,
    false);
  /* Check if we have a clock */
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_CLOCK, 1, 1, true);

  /* Check if we have 1 or 2 or 3 outputs */
  size_t num_output_ports =
    circuit_lib
      .model_ports_by_type(circuit_model, CIRCUIT_MODEL_PORT_OUTPUT, true)
      .size();
  if ((1 != num_output_ports) && (2 != num_output_ports) &&
      (3 != num_output_ports)) {
    VTR_LOG_ERROR(
      "Configuration flip-flop '%s' must have either 1 or 2 or 3 %s "
      "ports!\n\tAmong which:\n\t\tthe first port is the manadatory regular "
      "data output (e.g., Q) and \n\t\tthe second port could be the inverted "
      "data output which can optionally be enabled by configure-enable signal "
      "(e.g., QN or cgf_en_QN) and \n\t\tthe third port could be the data "
      "output which can optionally be enabled by configure-enable signal "
      "(e.g., cgf_en_Q)\n",
      circuit_lib.model_name(circuit_model).c_str(),
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_OUTPUT, num_output_ports, 1,
    false);
...
size_t check_bl_ccff_circuit_model_ports(const CircuitLibrary& circuit_lib,
  ...
  if (1 != num_input_ports) {
    VTR_LOG_ERROR(
      "Configuration flip-flop for BL shift register '%s' must have 1 %s "
      "port!\n",
      circuit_lib.model_name(circuit_model).c_str(),
      CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(CIRCUIT_MODEL_PORT_INPUT)]);
    num_err++;
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_BL, 1, 1, false);
...
size_t check_wl_ccff_circuit_model_ports(const CircuitLibrary& circuit_lib,
  ...
  if (1 != num_input_ports) {
    VTR_LOG_ERROR(
      "Configuration flip-flop for WL shift register '%s' must have 1 %s "
      "port!\n",
      circuit_lib.model_name(circuit_model).c_str(),
      CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(CIRCUIT_MODEL_PORT_INPUT)]);
    num_err++;
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_CLOCK, 2, 1, true);
  ...
  if (0 < circuit_lib
            .model_ports_by_type(circuit_model, CIRCUIT_MODEL_PORT_WLR, true)
            .size()) {
    num_err += check_one_circuit_model_port_type_and_size_required(
      circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_WLR, 1, 1, false);
  }
...
size_t check_sram_circuit_model_ports(const CircuitLibrary& circuit_lib,
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_OUTPUT, 2, 1, false);
  ...
  num_err += check_one_circuit_model_port_type_and_size_required(
    circuit_lib, circuit_model, CIRCUIT_MODEL_PORT_WL, 1, 1, false);
 ...
 ***********************************************************************/
static size_t check_circuit_library_ports(const CircuitLibrary& circuit_lib) {
  size_t num_err = 0;
  ...
  for (const auto& port : circuit_lib.ports()) {
    if ((circuit_lib.port_is_global(port)) &&
      ...
      VTR_LOG_ERROR(
        "Circuit port (type=%s) of model (name=%s) is defined as global but "
        "not an input/output port!\n",
  ...
  for (const auto& port : circuit_lib.ports()) {
    if ((circuit_lib.port_is_global(port)) &&
      ...
      VTR_LOG_ERROR(
        "Circuit port (type=%s) of model (name=%s) is defined as global output "
        "port but not an I/O!\n",
        CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(circuit_lib.port_type(port))],
        circuit_lib.model_name(port).c_str());
      num_err++;
  ...
  for (const auto& port : circuit_lib.ports()) {
    if (((circuit_lib.port_is_set(port)) || (circuit_lib.port_is_reset(port)) ||
         (circuit_lib.port_is_config_enable(port))) &&
        (!circuit_lib.port_is_global(port))) {
      VTR_LOG_ERROR(
        "Circuit port (type=%s) of model (name=%s) is defined as a "
        "set/reset/config_enable port but it is not global!\n",
        CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(circuit_lib.port_type(port))],
        circuit_lib.model_name(port).c_str());
      num_err++;
    }
  ...
  for (const auto& port : circuit_lib.ports()) {
    ...
    if (circuit_lib.port_tri_state_map(port).length() ==
        circuit_lib.port_size(port)) {
      continue; /* Sizes match, go to the next */
    }
    /* We have a problem here, sizes do not match, leave a message and raise the
     * error flag */
    VTR_LOG_ERROR(
      "Tri-state map (=%s) of circuit port (type=%s) of model (name=%s) does "
      "not match the port size (=%lu)!\n",
      circuit_lib.port_tri_state_map(port).c_str(),
      CIRCUIT_MODEL_PORT_TYPE_STRING[size_t(circuit_lib.port_type(port))],
      circuit_lib.model_name(port).c_str(), circuit_lib.port_size(port));
    num_err++;
  }

  /* Check all the global ports which sare the same name also share the same
   * attributes: default_value, is_config, is_reset, is_set etc.
   */
  ...
  for (size_t iport = 0; iport < global_ports.size() - 1; ++iport) {
    for (size_t jport = iport + 1; jport < global_ports.size(); ++jport) {
      ...
      if (0 != circuit_lib.port_prefix(global_ports[iport])
                 .compare(circuit_lib.port_prefix(global_ports[jport]))) {
        continue;
      }

      /* Check if a same port share the same attributes */
      CircuitModelId iport_parent_model =
        circuit_lib.port_parent_model(global_ports[iport]);
      CircuitModelId jport_parent_model =
        circuit_lib.port_parent_model(global_ports[jport]);

      if (circuit_lib.port_default_value(global_ports[iport]) !=
          circuit_lib.port_default_value(global_ports[jport])) {
        VTR_LOG_ERROR(
          "Global ports %s from circuit model %s and %s share the same name "
      ...
      if (circuit_lib.port_is_reset(global_ports[iport]) !=
          circuit_lib.port_is_reset(global_ports[jport])) {
        VTR_LOG_ERROR(
          "Global ports %s from circuit model %s and %s share the same name "
      ...
      if (circuit_lib.port_is_set(global_ports[iport]) !=
          circuit_lib.port_is_set(global_ports[jport])) {
        VTR_LOG_ERROR(
          "Global ports %s from circuit model %s and %s share the same name "
      ...
      if (circuit_lib.port_is_config_enable(global_ports[iport]) !=
          circuit_lib.port_is_config_enable(global_ports[jport])) {
        VTR_LOG_ERROR(
          "Global ports %s from circuit model %s and %s share the same name "
      ...
      if (circuit_lib.port_is_prog(global_ports[iport]) !=
          circuit_lib.port_is_prog(global_ports[jport])) {
        VTR_LOG_ERROR(
          "Global ports %s from circuit model %s and %s share the same name "
          "but have different is_prog attributes!\n",
          circuit_lib.port_prefix(global_ports[iport]).c_str(),
          circuit_lib.model_name(iport_parent_model).c_str(),
          circuit_lib.model_name(jport_parent_model).c_str());
        num_err++;
 ...
 ***********************************************************************/
static int check_power_gated_circuit_model(
  const CircuitLibrary& circuit_lib, const CircuitModelId& circuit_model) {
  int num_err = 0;
  ...
  if (2 > global_ports.size()) {
    VTR_LOGF_ERROR(__FILE__, __LINE__,
                   "Expect at least two global ports (a pair of EN/Enb) for "
                   "circuit model '%s' which is power-gated!\n",
                   circuit_lib.model_name(circuit_model).c_str());
    num_err++;
  }
  ...
  if (2 != num_config_enable_ports) {
    VTR_LOGF_ERROR(__FILE__, __LINE__,
                   "Circuit model '%s' is power-gated. Two config-enable "
                   "global ports are required!\n",
                   circuit_lib.model_name(circuit_model).c_str());
    num_err++;
  }
  ...
  if (false == circuit_lib.valid_circuit_port_id(en_port)) {
    VTR_LOGF_ERROR(__FILE__, __LINE__,
                   "Fail to find an enable port for the circuit model '%s' is "
                   "power-gated!\n",
                   circuit_lib.model_name(circuit_model).c_str());
  }
  if (false == circuit_lib.valid_circuit_port_id(enb_port)) {
    VTR_LOGF_ERROR(__FILE__, __LINE__,
                   "Fail to find an inverted enable port for the circuit model "
                   "'%s' is power-gated!\n",
                   circuit_lib.model_name(circuit_model).c_str());
  }
 ...
 ***********************************************************************/
static int check_power_gated_circuit_models(const CircuitLibrary& circuit_lib) {
  int num_err = 0;
 ...
 ***********************************************************************/
static size_t check_io_circuit_model(const CircuitLibrary& circuit_lib) {
  size_t num_err = 0;
   ...
   *  - At least 1 regular port that is non-global which is connected to global
  ...
  for (const auto& io_model : circuit_lib.models_by_type(CIRCUIT_MODEL_IOPAD)) {
     ...
     */
    if ((false == has_data_io) && (false == has_data_input_only_io) &&
    ...
    if (false == has_internal_connection) {
      VTR_LOGF_ERROR(__FILE__, __LINE__,
                     "I/O circuit model '%s' does not have any port connected "
                     "to FPGA core!\n",
                     circuit_lib.model_name(io_model).c_str());
      num_err++;
    }
 ...
 * 10. We must have default circuit models for these types: MUX, channel wires
 *and wires
 *
 * Note:
...
bool check_circuit_library(const CircuitLibrary& circuit_lib) {
  ...
  /* 1. Circuit models have unique names
   * For each circuit model, we always make sure it does not share any name with
   * any circuit model locating after it
   */
  num_err += check_circuit_library_unique_names(circuit_lib);

  /* 2. Circuit models have unique prefix
   * For each circuit model, we always make sure it does not share any prefix
   * with any circuit model locating after it
   */
   ...
   * [b] For each MUX, we must have at least an input, an output, and an SRAM
   * port
   */
  ...
  num_err += check_circuit_model_port_required(circuit_lib, CIRCUIT_MODEL_CCFF,
                                               ccff_port_types_required);
  ...
  num_err += check_circuit_model_port_required(circuit_lib, CIRCUIT_MODEL_FF,
                                               ff_port_types_required);
  ...
  num_err += check_circuit_model_port_required(circuit_lib, CIRCUIT_MODEL_LUT,
                                               lut_port_types_required);

  /* 10. For each type of circuit models that are define, we must have 1 default
   * model We must have default circuit models for these types: MUX, channel
   * wires and wires
   */
  num_err += check_default_circuit_model_by_types(circuit_lib);
  num_err +=
    check_required_default_circuit_model(circuit_lib, CIRCUIT_MODEL_MUX);
  num_err +=
    check_required_default_circuit_model(circuit_lib, CIRCUIT_MODEL_CHAN_WIRE);
  num_err +=
    check_required_default_circuit_model(circuit_lib, CIRCUIT_MODEL_WIRE);
