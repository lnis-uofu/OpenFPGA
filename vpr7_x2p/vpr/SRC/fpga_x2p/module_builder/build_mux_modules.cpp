...
/* TODO: we should have a header file for naming constexpr only */
...
void build_rram_mux_branch_module(ModuleManager& module_manager,
  ...
  std::vector<CircuitPortId> mux_input_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_INPUT, true);
  /* Get the output ports from the mux */
  std::vector<CircuitPortId> mux_output_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_OUTPUT, true);
  /* Get the BL and WL ports from the mux */
  std::vector<CircuitPortId> mux_blb_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_BLB, true);
  std::vector<CircuitPortId> mux_wl_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_WL, true);
...
void build_cmos_mux_module(ModuleManager& module_manager,
  ...
  std::vector<CircuitPortId> mux_global_ports = circuit_lib.model_global_ports_by_type(mux_model, SPICE_MODEL_PORT_INPUT, true, true);
  /* Get the input ports from the mux */
  std::vector<CircuitPortId> mux_input_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_INPUT, true);
  /* Get the output ports from the mux */
  std::vector<CircuitPortId> mux_output_ports = circuit_lib.model_ports_by_type(mux_model, SPICE_MODEL_PORT_OUTPUT, true);
  ...
  if ((SPICE_MODEL_MUX == circuit_lib.model_type(mux_model))
    || ((SPICE_MODEL_LUT == circuit_lib.model_type(mux_model))
       && (false == circuit_lib.is_lut_fracturable(mux_model))) ) {
    VTR_ASSERT(1 == mux_output_ports.size());
    VTR_ASSERT(1 == circuit_lib.port_size(mux_output_ports[0])); 
  } else {
    VTR_ASSERT_SAFE( (SPICE_MODEL_LUT == circuit_lib.model_type(mux_model)) 
