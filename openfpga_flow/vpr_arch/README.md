...
- <wide>\_<frac>\_[ff2]dsp<dsp\_size>reg<bus>: If Digital Signal Processor (DSP) is used or not. If used, the input size should be clarified here. The [ff2] prefix indicates that there are direct connection (hard-wired routing) between neighbour CLB and DSP blocks, which provides high-speed connection for DSP data paths. Bus keyword indicates that bus-based routing is enabled to reduce number of configuration bits.
