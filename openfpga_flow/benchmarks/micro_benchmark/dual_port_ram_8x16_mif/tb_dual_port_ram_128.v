...
module tb_dual_port_ram_128();
    ...
    // Task to perform a single read access
    task read_ram(input [2:0] addr);
        begin
            @(posedge clk);
            raddr <= addr;
            ren <= 1'b1;
            @(posedge clk);
            #1; // Wait small delay after edge for output settling
            ren <= 1'b0;
        end
    endtask
    ...
    // Main Test Stimulus
    initial begin
        // Step 1: Create the initialization file dynamically for the simulator
        // Matches the target MIF structure provided
        integer f;
        f = $fopen(MEM_FILE, "w");
        $fwrite(f, "0 138F\n");
        $fwrite(f, "1 0020\n");
