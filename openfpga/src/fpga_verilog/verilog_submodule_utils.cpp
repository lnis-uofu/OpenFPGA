...
void print_verilog_submodule_timing(std::fstream& fp,
  ...
  print_verilog_comment(
    fp, std::string("------ BEGIN Pin-to-pin Timing constraints -----"));
  fp << "\tspecify" << std::endl;

  /* Read out pin-to-pin delays by finding out all the edges belonging to a
   * circuit model */
  for (const auto& timing_edge :
       circuit_lib.timing_edges_by_model(circuit_model)) {
    ...
    BasicPort src_port_info(circuit_lib.port_lib_name(src_port), src_pin,
                            src_pin);
    ...
    BasicPort sink_port_info(circuit_lib.port_lib_name(sink_port), sink_pin,
                             sink_pin);
    ...
    fp << "(" << std::setprecision(FLOAT_PRECISION)
       << circuit_lib.timing_edge_delay(timing_edge, CIRCUIT_MODEL_DELAY_RISE) /
            VERILOG_SIM_TIMESCALE;
    fp << ", ";
    fp << std::setprecision(FLOAT_PRECISION)
       << circuit_lib.timing_edge_delay(timing_edge, CIRCUIT_MODEL_DELAY_FALL) /
            VERILOG_SIM_TIMESCALE
       << ")";
    fp << ";" << std::endl;
  }
  ...
  print_verilog_comment(
    fp, std::string("------ END Pin-to-pin Timing constraints -----"));
  fp << "`endif" << std::endl;
}
...
void add_user_defined_verilog_modules(ModuleManager& module_manager,
  ...
  for (const auto& model : circuit_lib.models()) {
     ...
     */
    ModuleId module_id =
      module_manager.find_module(circuit_lib.model_name(model));
...
static void print_one_verilog_template_module(
  ...
  print_verilog_comment(
    fp, std::string("----- Internal logic should start here -----"));
...
void print_verilog_submodule_templates(const ModuleManager& module_manager,
  ...
  std::string verilog_fname(submodule_dir +
