...
static void print_verilog_top_random_testbench_ports(
  ...
  for (const BasicPort& clock_port : clock_ports) {
    fp << "\t"
       << generate_verilog_port(VERILOG_PORT_REG, clock_port, true,
                                little_endian)
       << ";" << std::endl;
  }
  ...
  for (const AtomBlockId& atom_blk : atom_ctx.netlist().blocks()) {
    ...
    fp << "\t@(negedge "
       << generate_verilog_port(VERILOG_PORT_CONKT, clock_port, true,
                                little_endian)
       << ");" << std::endl;
    print_verilog_register_connection(fp, reset_port, reset_port, little_endian,
                                      true);
    fp << "\tend" << std::endl;
  }
...
void print_verilog_random_top_testbench(
  ...
  print_verilog_testbench_clock_stimuli(
    fp, pin_constraints, simulation_parameters, clock_ports, little_endian);
  ...
  if (!options.no_self_checking()) {
    print_verilog_testbench_check(
      ...
      clock_port_names, std::string(DEFAULT_CLOCK_NAME), little_endian);
  }
  ...
  print_verilog_timeout_and_vcd(
    ...
    simulation_time, options.no_self_checking(), little_endian);
