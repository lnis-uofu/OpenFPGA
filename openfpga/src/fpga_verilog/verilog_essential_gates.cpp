...
static void print_verilog_power_gated_invbuf_body(
  ...
  fp << "\treg " << circuit_lib.port_lib_name(output_port) << "_reg;"
     << std::endl;
  ...
  for (const auto& power_gate_port : power_gate_ports) {
    ...
    for (const auto& power_gate_pin : circuit_lib.pins(power_gate_port)) {
      ...
      /* Power-gated signal are disable during operating, enabled during
       * configuration, Therefore, we need to reverse them here
       */
   ...
   * 1. If this is an inverter or an tapered(multi-stage) buffer with odd number
   * of stages, we invert the input to output
   * 2. If this is a buffer or an tapere(multi-stage) buffer with even number of
   * stages, we wire the input to output
   */
  ...
  fp << "\t\t\tassign " << circuit_lib.port_lib_name(output_port)
     << "_reg = 1'bz;" << std::endl;
  fp << "\t\tend" << std::endl;
  fp << "\tend" << std::endl;
  fp << "\tassign " << circuit_lib.port_lib_name(output_port) << " = "
     << circuit_lib.port_lib_name(output_port) << "_reg;" << std::endl;
}
...
static void print_verilog_invbuf_body(std::fstream& fp,
  ...
  fp << "\tassign " << circuit_lib.port_lib_name(output_port) << " = ("
     << circuit_lib.port_lib_name(input_port) << " === 1'bz)? $random : ";
...
static void print_verilog_invbuf_module(
  ...
  if (true == circuit_lib.is_power_gated(circuit_model)) {
    ...
    print_verilog_power_gated_invbuf_body(fp, circuit_lib, circuit_model,
                                          input_ports[0], output_ports[0],
                                          global_ports);
  } else {
    /* Output Verilog codes for a regular inverter */
    print_verilog_invbuf_body(fp, circuit_lib, circuit_model, input_ports[0],
                              output_ports[0]);
  }

  /* Print timing info */
  print_verilog_submodule_timing(fp, circuit_lib, circuit_model, little_endian);
...
static void print_verilog_passgate_module(
  ...
  fp << circuit_lib.port_lib_name(input_ports[1]) << " ? "
     << circuit_lib.port_lib_name(input_ports[0]);
  fp << " : 1'bz;" << std::endl;
...
static void print_verilog_and_or_gate_body(
  ...
  print_verilog_comment(
    fp, std::string("----- Verilog codes of a " +
                    std::to_string(input_ports.size()) + "-input " +
                    std::to_string(output_ports.size()) +
                    "-output AND gate -----"));
...
static void print_verilog_gate_module(
  ...
  switch (circuit_lib.gate_type(circuit_model)) {
    case CIRCUIT_MODEL_GATE_AND:
    case CIRCUIT_MODEL_GATE_OR:
      print_verilog_and_or_gate_body(fp, circuit_lib, circuit_model,
                                     input_ports, output_ports, little_endian);
      break;
