 ...
 * of a given node.
 * We will walkthrough the input edges of a node and see if there is any
 *duplication
 **********************************************************************/
static bool check_lb_rr_graph_node_duplicated_edges(
  const LbRRGraph& lb_rr_graph, const LbRRNodeId& node) {
  ...
  for (const auto edge : lb_rr_graph.node_in_edges(node)) {
    ...
    auto result = edge_counter[lb_rr_graph.edge_mode(edge)].insert(
      std::pair<LbRREdgeId, size_t>(edge, 1));
...
static bool check_lb_rr_graph_source_nodes(const LbRRGraph& lb_rr_graph) {
  ...
  for (auto node : lb_rr_graph.nodes()) {
    ...
    if ((0 != lb_rr_graph.node_in_edges(node).size()) ||
      ...
      /* Print a warning! */
      VTR_LOG_WARN(
        "Source node %d is invalid (should have zero fan-in and non-zero "
        "fan-out)!\n",
        size_t(node));
      VTR_LOG_WARN("Node details for debugging:\n");
      print_lb_rr_node(lb_rr_graph, node);
...
static bool check_lb_rr_graph_sink_nodes(const LbRRGraph& lb_rr_graph) {
  ...
  for (auto node : lb_rr_graph.nodes()) {
    ...
    if ((0 == lb_rr_graph.node_in_edges(node).size()) ||
      ...
      /* Print a warning! */
      VTR_LOG_WARN(
        "Sink node %s is invalid (should have non-zero fan-in and zero "
        "fan-out)!\n",
        node);
      VTR_LOG_WARN("Node details for debugging:\n");
      print_lb_rr_node(lb_rr_graph, node);
...
bool check_lb_rr_graph(const LbRRGraph& lb_rr_graph) {
  ...
  if (0 < num_err) {
    VTR_LOG_WARN(
      "Checked Logical tile Routing Resource graph with %d errors !\n",
      num_err);
  }
