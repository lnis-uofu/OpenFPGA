...
std::vector<PhysicalPb::FixedBitstreamInfo> PhysicalPb::fixed_bitstreams(
  const PhysicalPbId& pb) const {
  VTR_ASSERT(true == valid_pb_id(pb));
  return fixed_bitstreams_[pb];
}

std::vector<PhysicalPb::FixedBitstreamInfo>
PhysicalPb::fixed_mode_select_bitstreams(const PhysicalPbId& pb) const {
  VTR_ASSERT(true == valid_pb_id(pb));
...
void PhysicalPb::set_mode_bits(const PhysicalPbId& pb,
  ...
  if (mode_bits.size() != mode_bits_[pb].size()) {
    VTR_LOG_ERROR(
      ...
      name(pb).c_str(), pb_graph_node(pb)->hierarchical_type_name().c_str());
  ...
  for (size_t ibit = 0; ibit < mode_bits.size(); ibit++) {
    if (mode_bits[ibit] == '1' || mode_bits[ibit] == '0') {
      ...
      VTR_LOG_ERROR("Invalid mode bit '%c' for pb '%s' based on pb_graph_node '%s'! Expect [1|0|x]\n",
                    mode_bits[ibit],
                    name(pb).c_str(),
