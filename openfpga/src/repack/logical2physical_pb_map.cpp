...
t_pb_graph_node* Logical2PhysicalPbMap::pb_graph_node(t_pb_graph_node* lgk_pb_graph_node) const {
  if (empty()) {
...
t_pb_graph_pin* Logical2PhysicalPbMap::pb_graph_pin(t_pb_graph_pin* lgk_pb_graph_pin) const {
  if (empty()) {
...
bool Logical2PhysicalPbMap::build_pb_graph_input_pin_map(t_pb_graph_node* lgk_pb_graph_node,
                                                         t_pb_graph_node* phy_pb_graph_node,
                                                         const bool& verbose) {
  if (lgk_pb_graph_node->num_input_ports != phy_pb_graph_node->num_input_ports) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' has different number of input ports (%d) than physical pb_graph_node '%s' whose number of input ports is (%d). The two cannot be considered as equivalent sites for repacking\n",
  ...
  for (int iport = 0; iport < lgk_pb_graph_node->num_input_ports; ++iport) {
    if (lgk_pb_graph_node->num_input_pins[iport] != phy_pb_graph_node->num_input_pins[iport]) {
      VTR_LOG_ERROR("Logical pb_graph_node '%s' has a input port which has a different number of pins (%d) than physical pb_graph_node '%s' whose number of pins is (%d). The two cannot be considered as equivalent sites for repacking\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
    ...
    for (int ipin = 0; ipin < lgk_pb_graph_node->num_input_pins[iport]; ++ipin) {
      if (std::string(lgk_pb_graph_node->input_pins[iport][ipin].port->name)
        ...
        VTR_LOG_ERROR("Logical pb_graph_node '%s' has a input port which has a different name '%s' than physical pb_graph_node '%s' whose port name is '%s'. The two cannot be considered as equivalent sites for repacking\n",
        lgk_pb_graph_node->hierarchical_type_name().c_str(),
      ...
      pb_graph_pin_map_[&lgk_pb_graph_node->input_pins[iport][ipin]] = &phy_pb_graph_node->input_pins[iport][ipin];
    }
  }
  VTR_LOGV(verbose, "Logical pb_graph_node '%s' is equivalent in input ports to physical pb_graph_node '%s'\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
      phy_pb_graph_node->hierarchical_type_name().c_str());
  return true;
}

bool Logical2PhysicalPbMap::build_pb_graph_output_pin_map(t_pb_graph_node* lgk_pb_graph_node,
                                                         t_pb_graph_node* phy_pb_graph_node,
                                                         const bool& verbose) {
  if (lgk_pb_graph_node->num_output_ports != phy_pb_graph_node->num_output_ports) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' has different number of output ports (%d) than physical pb_graph_node '%s' whose number of output ports is (%d). The two cannot be considered as equivalent sites for repacking\n",
    lgk_pb_graph_node->hierarchical_type_name().c_str(),
  ...
  for (int iport = 0; iport < lgk_pb_graph_node->num_output_ports; ++iport) {
    if (lgk_pb_graph_node->num_output_pins[iport] != phy_pb_graph_node->num_output_pins[iport]) {
      VTR_LOG_ERROR("Logical pb_graph_node '%s' has a output port which has a different number of pins (%d) than physical pb_graph_node '%s' whose number of pins is (%d). The two cannot be considered as equivalent sites for repacking\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
    ...
    for (int ipin = 0; ipin < lgk_pb_graph_node->num_output_pins[iport]; ++ipin) {
      if (std::string(lgk_pb_graph_node->output_pins[iport][ipin].port->name)
        ...
        VTR_LOG_ERROR("Logical pb_graph_node '%s' has a output port which has a different name '%s' than physical pb_graph_node '%s' whose port name is '%s'. The two cannot be considered as equivalent sites for repacking\n",
        lgk_pb_graph_node->hierarchical_type_name().c_str(),
      ...
      /* Build the map, sanity checks passed */
      pb_graph_pin_map_[&lgk_pb_graph_node->output_pins[iport][ipin]] = &phy_pb_graph_node->output_pins[iport][ipin];
    }
  }
  VTR_LOGV(verbose, "Logical pb_graph_node '%s' is equivalent in output ports to physical pb_graph_node '%s'\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
      phy_pb_graph_node->hierarchical_type_name().c_str());
  return true;
}

bool Logical2PhysicalPbMap::build_pb_graph_clock_pin_map(t_pb_graph_node* lgk_pb_graph_node,
                                                         t_pb_graph_node* phy_pb_graph_node,
                                                         const bool& verbose) {
  if (lgk_pb_graph_node->num_clock_ports != phy_pb_graph_node->num_clock_ports) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' has different number of clock ports (%d) than physical pb_graph_node '%s' whose number of clock ports is (%d). The two cannot be considered as equivalent sites for repacking\n",
    lgk_pb_graph_node->hierarchical_type_name().c_str(),
  ...
  for (int iport = 0; iport < lgk_pb_graph_node->num_clock_ports; ++iport) {
    if (lgk_pb_graph_node->num_clock_pins[iport] != phy_pb_graph_node->num_clock_pins[iport]) {
      VTR_LOG_ERROR("Logical pb_graph_node '%s' has a clock port which has a different number of pins (%d) than physical pb_graph_node '%s' whose number of pins is (%d). The two cannot be considered as equivalent sites for repacking\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
    ...
    for (int ipin = 0; ipin < lgk_pb_graph_node->num_clock_pins[iport]; ++ipin) {
      if (std::string(lgk_pb_graph_node->clock_pins[iport][ipin].port->name)
        ...
        VTR_LOG_ERROR("Logical pb_graph_node '%s' has a clock port which has a different name '%s' than physical pb_graph_node '%s' whose port name is '%s'. The two cannot be considered as equivalent sites for repacking\n",
        lgk_pb_graph_node->hierarchical_type_name().c_str(),
      ...
      /* Build the map, sanity checks passed */
      pb_graph_pin_map_[&lgk_pb_graph_node->clock_pins[iport][ipin]] = &phy_pb_graph_node->clock_pins[iport][ipin];
    }
  }
  VTR_LOGV(verbose, "Logical pb_graph_node '%s' is equivalent in clock ports to physical pb_graph_node '%s'\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
      phy_pb_graph_node->hierarchical_type_name().c_str());
  return true;
}
...
bool Logical2PhysicalPbMap::rec_build_pb_map(t_pb_graph_node* lgk_pb_graph_node,
  ...
  if (lgk_pb_graph_node->placement_index != phy_pb_graph_node->placement_index) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' has different relative index (%d) than physical pb_graph_node '%s' with  index (%d). The two cannot be considered as equivalent sites for repacking\n",
    lgk_pb_graph_node->hierarchical_type_name().c_str(),
  ...
  bool status = build_pb_graph_input_pin_map(lgk_pb_graph_node, phy_pb_graph_node, verbose);
  if (!status) {
    return false;
  }
  ...
  if (!lgk_pb_graph_node->is_primitive() && phy_pb_graph_node->is_primitive()) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' is a primitive one while physical pb_graph_node '%s' is not. The two cannot be considered as equivalent sites for repacking\n",
    lgk_pb_graph_node->hierarchical_type_name().c_str(),
    phy_pb_graph_node->hierarchical_type_name().c_str()); 
    return false;
  }
  if (lgk_pb_graph_node->is_primitive() && !phy_pb_graph_node->is_primitive()) {
    VTR_LOG_ERROR("Logical pb_graph_node '%s' is not a primitive one while physical pb_graph_node '%s' is. The two cannot be considered as equivalent sites for repacking\n",
    lgk_pb_graph_node->hierarchical_type_name().c_str(),
    phy_pb_graph_node->hierarchical_type_name().c_str()); 
    return false;
  }
  ...
  if (lgk_pb_graph_node->is_primitive() && phy_pb_graph_node->is_primitive()) {
    ...
    VTR_LOGV(verbose, "Logical pb_graph_node '%s' is equivalent to physical pb_graph_node '%s'\n",
        lgk_pb_graph_node->hierarchical_type_name().c_str(),
        phy_pb_graph_node->hierarchical_type_name().c_str());
    return true; 
  }
  ...
  /* Go recursively */
  VTR_LOGV(verbose, "Go to compare children of logical pb_graph_node '%s' and physical pb_graph_node '%s' as neither are primitive nodes\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
      phy_pb_graph_node->hierarchical_type_name().c_str());
  ...
  for (int ipb = 0; ipb < lgk_pb_mode->num_pb_type_children; ipb++) {
    if (lgk_pb_mode->pb_type_children[ipb].num_pb != phy_pb_mode->pb_type_children[ipb].num_pb) {
      VTR_LOG_ERROR("Logical pb_graph_node '%s' contains a child pb_type '%s' whose count (%d) is different than physical pb_graph_node '%s' who has %d child pb_types in the same name. The two cannot be considered as equivalent sites for repacking\n",
      lgk_pb_graph_node->hierarchical_type_name().c_str(),
