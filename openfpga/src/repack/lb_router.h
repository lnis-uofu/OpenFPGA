 ...
 *  std::vector<LbRRNodeId> source_nodes =
 *<find_your_source_node_in_your_lb_rr_graph>(); std::vector<LbRRNodeId>
 *sink_nodes = <find_your_sink_node_in_your_lb_rr_graph>(); LbNetId net =
 *lb_router.create_net_to_route(source_nodes, sink_nodes);
 *  // Add more nets
 *
 *  // Initialize the modes to expand routing
 *  // This is a must-do before running the router in the purpose of
 *repacking!!! lb_router.set_physical_pb_modes(lb_rr_graph, device_annotation);
 *
 *  // Run the router
 *  bool route_success = lb_router.try_route(lb_rr_graph, atom_ctx.nlist,
 *verbose);
 *
 ...
 *  // Here is an example to check which nodes are mapped to the 'net' created
 *before std::vector<LbRRNodeId> routed_nodes = lb_router.net_routed_nodes(net);
 *
 *******************************************************************/
...
class LbRouter {
 ...
 public: /* Intra-Logic Block Routing Data Structures (by instance) */
  ...
  /**************************************************************************
   * Data structure forming the route tree of a net within one logic
   *cluster_ctx.blocks. A net is implemented using routing resource nodes. The
   *t_lb_trace data structure records one of the nodes used by the net and the
   *connections to other nodes
   ***************************************************************************/
  struct t_trace {
    LbRRNodeId current_node; /* current t_lb_type_rr_node used by net */
    std::vector<t_trace>
      next_nodes; /* index of previous edge that drives current node */
  };
  ...
  struct t_expansion_node {
    LbRRNodeId node_index; /* Index of logic cluster_ctx.blocks rr node this
                              expansion node represents */
    LbRRNodeId prev_index; /* Index of logic cluster_ctx.blocks rr node that
                              drives this expansion node */
    float cost;
  ...
  struct t_explored_node_stats {
    ...
    int enqueue_id;  /* ID used ot determine if this node has been pushed on
  ...
  // TODO: check if this hacky class memory reserve thing is still necessary, if
  // not, then delete
  /* Packing uses a priority queue that requires a large number of elements.
   * This backdoor
   * allows me to use a priority queue where I can pre-allocate the # of
   * elements in the underlying container for efficiency reasons.  Note: Must
   * use vector with this
   */
 ...
 public: /* Public accessors */
  ...
  /**
   * Find all the routing resource nodes that are over-used, which they are used
   * more than their capacity This function is call to collect the nodes and
   * router can reroute these net
   */
  std::vector<LbRRNodeId> find_congested_rr_nodes(
 ...
 public: /* Public mutators */
  ...
  void add_net_atom_pins(const NetId& net, const AtomPinId& src_pin,
  ...
  /* Set all the modes in routing status with the physical mode defined in
   * device annotation This method is used only in repacking for physical
   * logical blocks Do NOT use it during the general purpose packing
   */
   ...
   */
  bool try_route(const LbRRGraph& lb_rr_graph, const AtomNetlist& atom_nlist,
                 const bool& verbosity);

 private: /* Private accessors */
  /**
   * Report if the routing is successfully done on a logical block routing
   * resource graph
   */
  ...
  void rec_collect_trace_nodes(const t_trace* trace,
                               std::vector<LbRRNodeId>& routed_nodes) const;

 private: /* Private mutators */
  /*It is possible that a net may connect multiple times to a logically
   *equivalent set of primitive pins. The cluster router will only route one
   *connection for a particular net to the common sink of the equivalent pins.
   *
   *To work around this, we fix all but one of these duplicate connections to
   *route to specific pins, (instead of the common sink). This ensures a legal
   *routing is produced and that the duplicate pins are not 'missing' in the
   *clustered netlist.
   */
  void fix_duplicate_equivalent_pins(const AtomContext& atom_ctx,
                                     const LbRRGraph& lb_rr_graph);
  bool check_edge_for_route_conflicts(
    std::unordered_map<const t_pb_graph_node*, const t_mode*>& mode_map,
    const t_pb_graph_pin* driver_pin, const t_pb_graph_pin* pin);
  void commit_remove_rt(
  ...
  bool add_to_rt(t_trace* rt, const LbRRNodeId& node_index,
                 const NetId& irt_net);
  void add_source_to_rt(const NetId& inet, const size_t& isrc);
  void expand_rt_rec(t_trace* rt, const LbRRNodeId& prev_index,
                     const NetId& irt_net, const int& explore_id_index);
  void expand_rt(const NetId& inet, const NetId& irt_net, const size_t& isrc);
  void expand_edges(const LbRRGraph& lb_rr_graph, t_mode* mode,
                    const LbRRNodeId& cur_inode, float cur_cost,
                    int net_fanout);
  void expand_node(const LbRRGraph& lb_rr_graph,
                   const t_expansion_node& exp_node, const int& net_fanout);
