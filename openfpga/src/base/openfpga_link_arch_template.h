...
int link_arch_template(T& openfpga_ctx, const Command& cmd,
  ...
  /* TODO: The best to get the gsb version from RRGraphView when it is created. It can avoid mismatches between vpr options and here */
  e_gsb_version gsb_version = e_gsb_version::GSB_V1; /* Default GSB version is 1 */
  if (cmd_context.option_enable(cmd, opt_gsb_version)) {
    ...
    if (gsb_ver_str == std::string("none")) {
      ...
      VTR_LOG_ERROR("Invalid GSB version '%s'. Supported versions are [ none | 1 | 2 ].\n",
                    gsb_ver_str.c_str());
  ...
  status = annotate_post_routing_cluster_sync_results(
    g_vpr_ctx.clustering(), openfpga_ctx.mutable_vpr_clustering_annotation());
