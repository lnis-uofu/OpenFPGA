 ...
 * Apply final correction on the dont care bits by referring to the base mode bits from physical mode
 *******************************************************************/
static std::vector<bool> generate_mode_select_bitstream(
  const std::vector<char>& mode_bits, const std::vector<char>& base_mode_bits = std::vector<char>()) {
  std::vector<bool> mode_select_bitstream;

  /* In absence of base mode bits, all the mode bits must be valid!
   * If a base mode bits is provided, use it to correct the 'x' bits in the mode bits */
  if (base_mode_bits.empty()) {
    for (const char& mode_bit : mode_bits) {
      /* Error out for unexpected bits */
      if ('0' != mode_bit && '1' != mode_bit) {
        VTR_LOG_ERROR("Final mode bits still contain dont care bits! Expect [0 | 1]! Please ensure all the operating mode bits and physical mode bits can fully resolve dont care bits!\n");
    ...
    if (mode_bits.size() != base_mode_bits.size()) {
      VTR_LOG_ERROR("Physical mode bits (%lu) does not match the size of operating mode bits (%lu)!\n", base_mode_bits.size(), mode_bits.size());
      exit(openfpga::CMD_EXEC_FATAL_ERROR);
    }
...
static void build_primitive_bitstream(
  ...
  if (true == physical_pb.valid_pb_id(primitive_pb_id)) {
    mode_select_bitstream =
      generate_mode_select_bitstream(physical_pb.mode_bits(primitive_pb_id), device_annotation.pb_type_mode_bits(primitive_pb_type));
...
static void build_lut_bitstream(
  ...
  if (0 != lut_mode_select_ports.size()) {
    ...
    if (true == physical_pb.valid_pb_id(lut_pb_id)) {
      mode_select_bitstream =
        generate_mode_select_bitstream(physical_pb.mode_bits(lut_pb_id), device_annotation.pb_type_mode_bits(lut_pb_type));
