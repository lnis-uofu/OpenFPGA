...
static void build_switch_block_interc_bitstream(
  ...
  const size_t& chan_node_id, const std::string& unused_mux_config,
  const bool& verbose) {
  ...
  if ((0 == driver_rr_nodes.size()) || (0 == driver_rr_nodes.size())) {
    ...
    build_switch_block_mux_bitstream(
      ...
      gsb_edges.get_chan_node_in_edges(rr_gsb, in_edges, chan_side,
                                       chan_node_id),
      atom_ctx, device_annotation, routing_annotation, unused_mux_config, verbose);
...
static void build_connection_block_mux_bitstream(
  ...
  const size_t& ipin_index, const std::string& unused_mux_config, const bool& verbose) {
  RRNodeId src_rr_node = rr_gsb.get_ipin_node(cb_ipin_side, ipin_index);
  /* Find drive_rr_nodes*/
  std::vector<RREdgeId> driver_rr_edges = gsb_edges.get_ipin_node_in_edges(
    rr_gsb, in_edges, cb_ipin_side, ipin_index);
  ...
  VTR_ASSERT_MSG(1 <= driver_switches.size(),
                 ("There should be at least one driver switch for " +
                  std::to_string(size_t(src_rr_node)))
                   .c_str());
  /* Iterate over driver switches to find a common switch that can be used */
  CircuitModelId switch_index =
    device_annotation.rr_switch_circuit_model(driver_switches[0]);
  for (size_t i = 1; i < driver_switches.size(); ++i) {
    VTR_ASSERT_MSG(device_annotation.rr_switch_circuit_model(
                     driver_switches[i]) == switch_index,
                   ("All driver switches of " +
                    std::to_string(size_t(src_rr_node)) + " must be the same")
                     .c_str());
  }
  CircuitModelId mux_model =
    device_annotation.rr_switch_circuit_model(driver_switches[0]);
...
static void build_connection_block_interc_bitstream(
  ...
  if (1 == driver_rr_nodes.size()) {
    ...
    build_connection_block_mux_bitstream(
      ...
      rr_graph, in_edges, rr_gsb, gsb_edges, cb_ipin_side, ipin_index, unused_mux_config, verbose);
  } /*Nothing should be done else*/
}
...
static void build_connection_block_bitstream(
  ...
  const VprRoutingAnnotation& routing_annotation, const RRGraphView& rr_graph,
  const RRGraphInEdges& in_edges, const RRGSB& rr_gsb,
  const RRGSBEdges& gsb_edges, const e_rr_type& cb_type, const std::string& unused_mux_config,
  const bool& verbose) {
  ...
  for (size_t side = 0; side < cb_sides.size(); ++side) {
    ...
    for (size_t inode = 0; inode < rr_gsb.get_num_ipin_nodes(cb_ipin_side);
      ...
      build_connection_block_interc_bitstream(
        ...
        routing_annotation, rr_graph, in_edges, rr_gsb, gsb_edges, cb_ipin_side,
        inode, unused_mux_config, verbose);
    }
  }
}
...
static void build_connection_block_bitstreams(
  ...
  const bool& compact_routing_hierarchy, const e_rr_type& cb_type,
  const std::string& unused_mux_config, const bool& verbose) {
  ...
  for (size_t ix = 0; ix < cb_range.x(); ++ix) {
    for (size_t iy = 0; iy < cb_range.y(); ++iy) {
      ...
      build_connection_block_bitstream(
        ...
        device_rr_gsb.get_gsb_edges(ix, iy), cb_type, unused_mux_config, verbose);
...
void build_routing_bitstream(
  ...
  for (size_t ix = 0; ix < sb_range.x(); ++ix) {
    for (size_t iy = 0; iy < sb_range.y(); ++iy) {
      ...
      build_switch_block_bitstream(
        ...
        module_name_map, circuit_lib, mux_lib, atom_ctx, device_annotation,
        routing_annotation, rr_graph, in_edges, rr_gsb, gsb_edges,
        unused_mux_config, verbose);
  ...
  build_connection_block_bitstreams(
    ...
    routing_annotation, rr_graph, in_edges, device_rr_gsb,
    compact_routing_hierarchy, e_rr_type::CHANX, unused_mux_config, verbose);
  VTR_LOG("Done\n");
  ...
  build_connection_block_bitstreams(
    ...
    fabric_tile, circuit_lib, mux_lib, atom_ctx, device_annotation,
    routing_annotation, rr_graph, in_edges, device_rr_gsb,
    compact_routing_hierarchy, e_rr_type::CHANY, unused_mux_config, verbose);
  VTR_LOG("Done\n");
}
