...
static void build_switch_block_mux_bitstream(
  ...
  } else if (false == circuit_lib.mux_add_const_input(mux_model) && fix_unmapped_mux_selection){
    /* If
     * 1. output net is INVALID (unmapped)
     * 2. and we don't have a constant input,
     * 3. and the fix_unmapped_mux_selection flag is on,
...
static void build_switch_block_interc_bitstream(
  ...
  if ((0 == driver_rr_nodes.size()) || (0 == driver_rr_nodes.size())) {
    ...
    build_switch_block_mux_bitstream(
      ...
      device_annotation, routing_annotation, verbose, fix_unmapped_mux_selection);
...
static void build_switch_block_bitstream(
  ...
  const RRGSB& rr_gsb, bool verbose, bool fix_unmapped_mux_selection) {
  ...
  for (size_t side = 0; side < rr_gsb.get_num_sides(); ++side) {
    ...
    for (size_t itrack = 0;
      ...
      build_switch_block_interc_bitstream(
        ...
        routing_annotation, rr_gsb, side_manager.get_side(), itrack, verbose, fix_unmapped_mux_selection);
    }
  }
}
...
static void build_connection_block_interc_bitstream(
  ...
  if (1 == driver_rr_nodes.size()) {
    ...
    build_connection_block_mux_bitstream(
      ...
      rr_graph, rr_gsb, cb_ipin_side, ipin_index, verbose, fix_unmapped_mux_selection);
  } /*Nothing should be done else*/
}
...
static void build_connection_block_bitstreams(
  ...
  const t_rr_type& cb_type, bool verbose, bool fix_unmapped_mux_selection) {
...
void build_routing_bitstream(
  ...
  for (size_t ix = 0; ix < sb_range.x(); ++ix) {
    for (size_t iy = 0; iy < sb_range.y(); ++iy) {
      ...
      build_switch_block_bitstream(
        ...
        module_name_map, circuit_lib, mux_lib, atom_ctx, device_annotation,
        routing_annotation, rr_graph, rr_gsb, verbose, fix_unmapped_mux_selection);
  ...
  build_connection_block_bitstreams(
    ...
    CHANX, verbose, fix_unmapped_mux_selection);
  VTR_LOG("Done\n");
  ...
  build_connection_block_bitstreams(
    ...
    fabric_tile, circuit_lib, mux_lib, atom_ctx, device_annotation,
    routing_annotation, rr_graph, device_rr_gsb, compact_routing_hierarchy,
    CHANY, verbose, fix_unmapped_mux_selection);
  VTR_LOG("Done\n");
}
