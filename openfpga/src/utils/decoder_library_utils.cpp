/***************************************************************************************
 * This file includes most utilized functions for the DecoderLibrary data
 *structure
 ***************************************************************************************/
 ...
 *  Considering this fact, there are only num_of_outputs + 1 conditions to be
 *encoded. Therefore, the number of inputs is ceil(log(num_of_outputs+1)/log(2))
 ...
 *  Then we can use the function for mux local encoder to compute the address
 *size
 *
 ***************************************************************************************/
...
/***************************************************************************************
 * Find the size of data lines (BLs and WLs) for a memory decoder to access a
 *memory array As the memory cells are organized in an array with shared bit
 ...
 * is the dominant factor. It means that the BL data lines is strictly an
 *integeter close to the square root of the number of memory cells. For example,
 *203 memory cells leads to 15 BLs to control The WL data lines may not be
 *exactly the same as the number of BLs. Considering the example of 203 memory
 *cells again, when 15 BLs are used, we just need 203 / 15 = 13.5555 -> 14 WLs
 ***************************************************************************************/
...
DecoderId add_mux_local_decoder_to_library(DecoderLibrary& decoder_lib,
  ...
  DecoderId decoder_id =
    decoder_lib.find_decoder(addr_size, data_size, false, false, true, false);
