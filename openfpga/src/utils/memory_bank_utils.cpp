...
std::pair<int, int> compute_memory_bank_regional_configurable_child_x_range(
  ...
  std::pair<int, int> child_x_range(
    ...
    std::numeric_limits<int>::min());  // Deposit an invalid range first:
                                       // LSB->max(); MSB->min()
...
size_t find_module_ql_memory_bank_num_blwls(
  ...
  for (const std::string& config_port_name : config_port_names) {
    ...
    if (circuit_port_type != circuit_lib.port_type(circuit_lib.model_port(
                               sram_model, config_port_name))) {
      continue;
    }
...
std::map<int, size_t> compute_memory_bank_regional_bitline_numbers_per_tile(
  ...
  for (size_t child_id = 0;
    ...
    vtr::Point<int> coord =
      module_manager.region_configurable_child_coordinates(
        top_module, config_region)[child_id];
    num_bls_per_tile[coord.x()] =
      std::max(num_bls_per_tile[coord.x()],
               find_module_ql_memory_bank_num_blwls(
...
std::map<int, size_t> compute_memory_bank_regional_wordline_numbers_per_tile(
  ...
  for (size_t child_id = 0;
    ...
    vtr::Point<int> coord =
      module_manager.region_configurable_child_coordinates(
        top_module, config_region)[child_id];
    num_wls_per_tile[coord.y()] =
      std::max(num_wls_per_tile[coord.y()],
               find_module_ql_memory_bank_num_blwls(
                 module_manager, child_module, circuit_lib, sram_model,
...
std::map<int, size_t> compute_memory_bank_regional_blwl_start_index_per_tile(
  ...
  for (int iblwl = child_xy_range.first; iblwl <= child_xy_range.second;
       ++iblwl) {
    if (iblwl == child_xy_range.first) {
      ...
      if (result == num_blwls_per_tile.end()) {
        ...
        blwl_start_index_per_tile[iblwl] =
          result->second + blwl_start_index_per_tile[iblwl - 1];
      }
    }
  }
