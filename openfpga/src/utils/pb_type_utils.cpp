...
bool is_primitive_pb_type(t_pb_type* pb_type) {
  if (LUT_CLASS == pb_type->class_type) {
    ...
    VTR_ASSERT(
      (std::string("wire") == std::string(pb_type->modes[0].name)) &&
    ...
    VTR_ASSERT(
      (std::string("memory_slice") == std::string(pb_type->modes[0].name)) ||
      (std::string("memory_slice_1bit") ==
       std::string(pb_type->modes[0].name)));
    return true;
  }
...
t_pb_type* find_mode_child_pb_type(t_mode* mode, const char* child_name) {
  for (int i = 0; i < mode->num_pb_type_children; ++i) {
    if (std::string(child_name) ==
        std::string(mode->pb_type_children[i].name)) {
 ...
 *******************************************************************/
t_pb_type* try_find_pb_type_with_given_path(
  t_pb_type* top_pb_type, const std::vector<std::string>& target_pb_type_names,
  const std::vector<std::string>& target_pb_mode_names) {
  /* Ensure that number of parent names and modes matches */
  VTR_ASSERT_SAFE(target_pb_type_names.size() ==
                  target_pb_mode_names.size() + 1);
  ...
  for (size_t i = 0; i < target_pb_type_names.size() - 1; ++i) {
    ...
    t_mode* cur_mode =
      find_pb_type_mode(cur_pb_type, target_pb_mode_names[i].c_str());
    if (nullptr == cur_mode) {
      return nullptr;
    }
    /* Go to the next level of pb_type */
    cur_pb_type =
      find_mode_child_pb_type(cur_mode, target_pb_type_names[i + 1].c_str());
 ...
 * If not found, return null pointer
 *******************************************************************/
t_interconnect* find_pb_mode_interconnect(t_mode* pb_mode,
                                          const char* interc_name) {
  for (int i = 0; i < pb_mode->num_interconnect; ++i) {
    if (std::string(interc_name) ==
        std::string(pb_mode->interconnect[i].name)) {
...
e_interconnect pb_interconnect_physical_type(t_interconnect* pb_interc,
  ...
  /* Initialize the interconnection type that will be implemented in SPICE
   * netlist*/
...
e_circuit_model_type pb_interconnect_require_circuit_model_type(
  ...
  VTR_ASSERT((MUX_INTERC == pb_interc_type) ||
             (DIRECT_INTERC == pb_interc_type));
 ...
 * As such, a few types of circuit port may be directed the same type of pb_type
 *port This is done to make the mapping much simpler than doing in the opposite
 *direction
 *******************************************************************/
enum PORTS circuit_port_require_pb_port_type(
  ...
  /* Other circuit model ports should not be found when mapping to the pb_type
   * ports */
 ...
 *******************************************************************/
std::vector<t_port*> find_pb_type_ports_match_circuit_model_port_type(
  t_pb_type* pb_type, const e_circuit_model_port_type& port_type,
  ...
  for (int iport = 0; iport < pb_type->num_ports; ++iport) {
    ...
    VTR_ASSERT(CircuitPortId::INVALID() !=
               vpr_device_annotation.pb_circuit_port(&(pb_type->ports[iport])));
    switch (port_type) {
 ...
 * <top_pb_type_name>[<mode_name>].<parent_pb_type_name> ...
 *<current_pb_type_name>
 *
...
std::string generate_pb_type_hierarchy_path(t_pb_type* cur_pb_type) {
  ...
  while (1) {
    ...
    hie_name = std::string("[") + std::string(parent_mode->name) +
               std::string("].") + hie_name;
