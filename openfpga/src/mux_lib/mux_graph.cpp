...
std::vector<MuxNodeId> MuxGraph::non_input_nodes() const {
  ...
  for (size_t level = 0; level < num_node_levels(); ++level) {
    for (size_t node_type = 0; node_type < size_t(NUM_MUX_NODE_TYPES);
         ++node_type) {
...
std::vector<MuxNodeId> MuxGraph::inputs() const {
  ...
  for (auto node_per_level : node_lookup_) {
    input_nodes.insert(input_nodes.end(),
                       node_per_level[MUX_INPUT_NODE].begin(),
                       node_per_level[MUX_INPUT_NODE].end());
  }
  return input_nodes;
}
...
std::vector<MuxNodeId> MuxGraph::outputs() const {
  ...
  for (auto node_per_level : node_lookup_) {
    output_nodes.insert(output_nodes.end(),
                        node_per_level[MUX_OUTPUT_NODE].begin(),
                        node_per_level[MUX_OUTPUT_NODE].end());
  }
  return output_nodes;
}

/* Find the edge between two MUX nodes */
std::vector<MuxEdgeId> MuxGraph::find_edges(const MuxNodeId& from_node,
                                            const MuxNodeId& to_node) const {
...
size_t MuxGraph::num_levels() const {
  /* need to check if the graph is valid or not */
  VTR_ASSERT_SAFE(valid_mux_graph());
  /* The num_levels by definition excludes the level for outputs, so a
   * deduection is applied */
...
std::vector<size_t> MuxGraph::branch_sizes() const {
  ...
  /* Visit each internal nodes/output nodes and find the the number of incoming
   * edges */
  for (auto node : node_ids_) {
    ...
    /* Nodes with the same number of incoming edges, indicate the same size of
     * branch circuit */
...
MuxGraph MuxGraph::subgraph(const MuxNodeId& root_node) const {
  ...
  for (auto edge_origin : this->node_in_edges_[root_node]) {
    ...
    /* Add edges */
    MuxEdgeId edge_subgraph = mux_graph.add_edge(
      node2node_map[from_node_origin], node2node_map[root_node]);
  ...
  for (auto edge_origin : this->node_in_edges_[root_node]) {
    ...
    if (it != mem2mem_map.end()) {
      ...
      /* configure the edge */
      mux_graph.edge_mem_ids_[edge2edge_map[edge_origin]] =
        mem2mem_map[mem_origin];
      continue;
    }
...
std::vector<MuxGraph> MuxGraph::build_mux_branch_graphs() const {
  std::map<size_t, bool>
    branch_done; /* A map showing the status of graph generation */
 ...
 */
MuxNodeId MuxGraph::node_id(const size_t& node_level,
                            const size_t& node_index_at_level) const {
...
vtr::vector<MuxMemId, bool> MuxGraph::decode_memory_bits(
  const MuxInputId& input_id, const MuxOutputId& output_id) const {
 ...
 */
MuxInputId MuxGraph::find_input_node_driven_by_output_node(
  const std::map<MuxMemId, bool>& memory_bits,
...
MuxEdgeId MuxGraph::add_edge(const MuxNodeId& from_node,
                             const MuxNodeId& to_node) {
...
void MuxGraph::build_multilevel_mux_graph(const size_t& mux_size,
                                          ...
                                          const size_t& num_inputs_per_branch,
   ...
   */
  std::vector<std::vector<MuxNodeId>>
    node_lookup; /* [num_levels][num_nodes_per_level] */
  ...
  for (size_t lvl = num_levels - 1;; --lvl) {
     ...
     */
    size_t node_cnt_per_level =
      0; /* A counter to record node indices at each level */
    for (MuxNodeId seed_node : node_lookup[lvl + 1]) {
      /* Add a new node and connect to seed_node, until we reach the
      ...
      for (size_t i = 0; i < num_inputs_per_branch; ++i) {
         ...
         * if number of inputs per branch is 2, it indicates a tree-like
         * multiplexer, every two edges will share one memory bit otherwise,
         * each edge corresponds to a memory bit
         */

        if (2 == num_inputs_per_branch) {
          MuxMemId mem_id = MuxMemId(lvl);
          set_edge_mem_id(edge, mem_id);
          /* If this is a second edge in the branch, we will assign it to an
           * inverted edge */
        ...
        /* Push the node to input list, and then remove the seed_node from the
         * list */
        input_node_ids.push_back(expand_node);
        /* Remove the node if the seed node is the list */
        std::vector<MuxNodeId>::iterator it =
          find(input_node_ids.begin(), input_node_ids.end(), seed_node);
        if (it != input_node_ids.end()) {
          input_node_ids.erase(it);
        }

        /* Check the number of input nodes, if already meet the demand, we can
         * finish here */
        if (mux_size != input_node_ids.size()) {
          continue; /* We need more inputs, keep looping */
        }

        /* The graph is done, we configure the input nodes and then we can
         * return */
...
void MuxGraph::add_fracturable_outputs(const CircuitLibrary& circuit_lib,
  ...
  for (const auto& port : circuit_lib.model_ports_by_type(
         circuit_model, CIRCUIT_MODEL_PORT_OUTPUT, false)) {
    ...
    for (const auto& output_idx : circuit_lib.port_lut_output_mask(port)) {
      ...
      /* Iterate over node and find the internal nodes, which match the
       * frac_level and output_idx */
...
void MuxGraph::build_mux_graph(const CircuitLibrary& circuit_lib,
  ...
  size_t impl_mux_size =
    find_mux_implementation_num_inputs(circuit_lib, circuit_model, mux_size);

  /* Depends on the mux size, the implemented multiplexer structure may change!
   */
  enum e_circuit_model_structure impl_structure =
    find_mux_implementation_structure(circuit_lib, circuit_model,
                                      impl_mux_size);
  ...
  switch (impl_structure) {
    case CIRCUIT_MODEL_STRUCTURE_TREE: {
      ...
      build_multilevel_mux_graph(
        impl_mux_size, num_levels, num_inputs_per_branch,
        circuit_lib.pass_gate_logic_model(circuit_model));
      break;
    }
    case CIRCUIT_MODEL_STRUCTURE_ONELEVEL: {
      build_onelevel_mux_graph(
        impl_mux_size, circuit_lib.pass_gate_logic_model(circuit_model));
      break;
    }
    case CIRCUIT_MODEL_STRUCTURE_MULTILEVEL: {
      /* Find the number of inputs per branch, this is not final */
      size_t num_inputs_per_branch = find_multilevel_mux_branch_num_inputs(
        impl_mux_size, circuit_lib.mux_num_levels(circuit_model));
...
void MuxGraph::invalidate_node_lookup() { node_lookup_.clear(); }

/* Invalidate (empty) the mem fast lookup*/
void MuxGraph::invalidate_mem_lookup() { mem_lookup_.clear(); }
...
/* validate an input id (from which data path signal will be progagated to the
 * output) */
...
bool MuxGraph::valid_node_lookup() const { return node_lookup_.empty(); }
