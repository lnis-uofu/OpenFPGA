 ...
 *******************************************************************/
static void rec_build_vpr_pb_graph_interconnect_physical_type_annotation(
  t_pb_graph_node* pb_graph_node, VprDeviceAnnotation& vpr_device_annotation,
  ...
  if (false == pb_graph_node->is_root()) {
    /* We only care the physical modes! But we have to find it through the
     * parent node */
    t_mode* child_physical_mode = vpr_device_annotation.physical_mode(
    ...
    for (int iport = 0; iport < pb_graph_node->num_input_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_input_pins[iport]; ++ipin) {
        for (int iedge = 0;
             iedge < pb_graph_node->input_pins[iport][ipin].num_input_edges;
             ++iedge) {
          t_interconnect* interc = pb_graph_node->input_pins[iport][ipin]
                                     .input_edges[iedge]
                                     ->interconnect;
    ...
    for (int iport = 0; iport < pb_graph_node->num_clock_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_clock_pins[iport]; ++ipin) {
        for (int iedge = 0;
             iedge < pb_graph_node->clock_pins[iport][ipin].num_input_edges;
             ++iedge) {
          t_interconnect* interc = pb_graph_node->clock_pins[iport][ipin]
                                     .input_edges[iedge]
                                     ->interconnect;
          /* Ensure that the interconnect is unique in the list */
          if (0 < interc_num_inputs.count(interc)) {
    ...
    for (int iport = 0; iport < pb_graph_node->num_input_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_input_pins[iport]; ++ipin) {
        ...
        for (const auto& pair : interc_num_inputs) {
          interc_num_inputs[pair.first] +=
            pb_graph_pin_inputs(&(pb_graph_node->input_pins[iport][ipin]),
                                pair.first)
              .size();
        }
      }
    ...
    for (int iport = 0; iport < pb_graph_node->num_clock_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_clock_pins[iport]; ++ipin) {
        ...
        for (const auto& pair : interc_num_inputs) {
          interc_num_inputs[pair.first] +=
            pb_graph_pin_inputs(&(pb_graph_node->clock_pins[iport][ipin]),
                                pair.first)
              .size();
        }
      }
    ...
    /* For each interconnect that has more than 1 input, we can infer the
     * physical type */
    for (const auto& pair : interc_num_inputs) {
      t_interconnect* interc = pair.first;
      size_t actual_interc_num_inputs = pair.second;
      /* If the number inputs for an interconnect is zero, this is a 0-driver
       * pin we just set 1 to use direct wires
       */
      if (0 == actual_interc_num_inputs) {
        actual_interc_num_inputs = 1;
      }

      e_interconnect interc_physical_type =
        pb_interconnect_physical_type(interc, actual_interc_num_inputs);
      if (interc_physical_type ==
          vpr_device_annotation.interconnect_physical_type(interc)) {
      ...
      VTR_LOGV(verbose_output,
               ...
               interc->name.c_str(), INTERCONNECT_TYPE_STRING[interc->type]);
  ...
  /* Before going recursive, we should check the interconnect between output
   * pins Note that this is NOT applicable to primitive pb_graph nodes!!!
   *
   *    pb_graph_node
   *    -------------------------------+
   *                                   |
  ...
  { /* Use a code block to use local variables freely */
    ...
    for (int iport = 0; iport < pb_graph_node->num_output_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_output_pins[iport]; ++ipin) {
        for (int iedge = 0;
             iedge < pb_graph_node->output_pins[iport][ipin].num_input_edges;
             ++iedge) {
          t_interconnect* interc = pb_graph_node->output_pins[iport][ipin]
                                     .input_edges[iedge]
                                     ->interconnect;
          /* Ensure that the interconnect is unique in the list */
          if (0 < interc_num_inputs.count(interc)) {
    ...
    for (int iport = 0; iport < pb_graph_node->num_output_ports; ++iport) {
      for (int ipin = 0; ipin < pb_graph_node->num_output_pins[iport]; ++ipin) {
        ...
        for (const auto& pair : interc_num_inputs) {
          interc_num_inputs[pair.first] +=
            pb_graph_pin_inputs(&(pb_graph_node->output_pins[iport][ipin]),
                                pair.first)
              .size();
        }
      }
  ...
  for (int ipb = 0; ipb < physical_mode->num_pb_type_children; ++ipb) {
    ...
    for (int jpb = 0; jpb < physical_mode->pb_type_children[ipb].num_pb;
         ++jpb) {
      rec_build_vpr_pb_graph_interconnect_physical_type_annotation(
...
void annotate_pb_graph_interconnect_physical_type(
  ...
  for (const t_logical_block_type& lb_type :
    ...
    rec_build_vpr_pb_graph_interconnect_physical_type_annotation(
      lb_type.pb_graph_head, vpr_device_annotation, verbose_output);
  }
}
 ...
 *******************************************************************/
static void rec_build_vpr_primitive_pb_graph_node_unique_index(
  t_pb_graph_node* pb_graph_node, VprDeviceAnnotation& vpr_device_annotation) {
  ...
  if (false == is_primitive_pb_type(pb_graph_node->pb_type)) {
    for (int imode = 0; imode < pb_graph_node->pb_type->num_modes; ++imode) {
      for (int ipb = 0;
           ipb < pb_graph_node->pb_type->modes[imode].num_pb_type_children;
           ++ipb) {
        /* Each child may exist multiple times in the hierarchy*/
        for (int jpb = 0;
             jpb <
             pb_graph_node->pb_type->modes[imode].pb_type_children[ipb].num_pb;
             ++jpb) {
          rec_build_vpr_primitive_pb_graph_node_unique_index(
            &(pb_graph_node->child_pb_graph_nodes[imode][ipb][jpb]),
 ...
 *   But these indices are not unique in the graph, as there are 20 LUTs in
 *total
 *******************************************************************/
static void annotate_primitive_pb_graph_node_unique_index(
  const DeviceContext& vpr_device_ctx,
  ...
  for (const t_logical_block_type& lb_type :
    ...
    rec_build_vpr_primitive_pb_graph_node_unique_index(lb_type.pb_graph_head,
                                                       vpr_device_annotation);
  }
}
 ...
 *******************************************************************/
static bool try_match_pb_graph_pin(
  t_pb_graph_pin* operating_pb_graph_pin, t_pb_graph_pin* physical_pb_graph_pin,
  const VprDeviceAnnotation& vpr_device_annotation) {
  /* If the parent ports of the two pins are not paired, fail */
  for (t_port* candidate_port :
       vpr_device_annotation.physical_pb_port(operating_pb_graph_pin->port)) {
     ...
     *
     * For example:
     *   We can align the operating_port[32] to physical_port[0] with an initial
     * offset which is -32
     *
     ...
     */
    int acc_offset = vpr_device_annotation.physical_pb_pin_offset(
                       operating_pb_graph_pin->port, candidate_port) +
                     vpr_device_annotation.physical_pb_port_offset(
                       operating_pb_graph_pin->port, candidate_port);
    int init_offset = vpr_device_annotation.physical_pb_pin_initial_offset(
      operating_pb_graph_pin->port, candidate_port);
    const BasicPort& physical_port_range =
      vpr_device_annotation.physical_pb_port_range(operating_pb_graph_pin->port,
                                                   candidate_port);
    if (physical_pb_graph_pin->pin_number !=
        operating_pb_graph_pin->pin_number +
          (int)physical_port_range.get_lsb() + init_offset + acc_offset) {
      /* Not the one we want, try the next candidate */
      continue;
    }
...
static void annotate_physical_pb_graph_pin(
  ...
  for (int iport = 0; iport < physical_pb_graph_node->num_input_ports;
    ...
    for (int ipin = 0; ipin < physical_pb_graph_node->num_input_pins[iport];
         ++ipin) {
      if (false == try_match_pb_graph_pin(
                     operating_pb_graph_pin,
                     &(physical_pb_graph_node->input_pins[iport][ipin]),
                     vpr_device_annotation)) {
        continue;
      }
      /* Reach here, it means the pins are matched by the annotation
       * requirements We can pair the pin and return
       */
      vpr_device_annotation.add_physical_pb_graph_pin(
        operating_pb_graph_pin,
        &(physical_pb_graph_node->input_pins[iport][ipin]));
      if (true == verbose_output) {
        print_success_bind_pb_graph_pin(
          operating_pb_graph_pin,
          vpr_device_annotation.physical_pb_graph_pin(operating_pb_graph_pin));
      }
      return;
    }
  }
  ...
  VTR_LOG_ERROR(
    "Fail to match a physical pin for '%s' from pb_graph_node '%s'!\n",
 ...
 *******************************************************************/
static void annotate_physical_pb_graph_node_pins(
   ...
   * and find the physical pins
   */
  for (int iport = 0; iport < operating_pb_graph_node->num_input_ports;
       ++iport) {
    for (int ipin = 0; ipin < operating_pb_graph_node->num_input_pins[iport];
         ++ipin) {
      annotate_physical_pb_graph_pin(
        &(operating_pb_graph_node->input_pins[iport][ipin]),
        physical_pb_graph_node, vpr_device_annotation, verbose_output);
    }
    ...
    t_pb_graph_pin* operating_pb_graph_pin =
      &(operating_pb_graph_node->input_pins[iport][0]);
    for (t_port* candidate_port :
         vpr_device_annotation.physical_pb_port(operating_pb_graph_pin->port)) {
      vpr_device_annotation.accumulate_physical_pb_port_rotate_offset(
        operating_pb_graph_pin->port, candidate_port);
    }
  }

  for (int iport = 0; iport < operating_pb_graph_node->num_output_ports;
       ++iport) {
    for (int ipin = 0; ipin < operating_pb_graph_node->num_output_pins[iport];
         ++ipin) {
      annotate_physical_pb_graph_pin(
        &(operating_pb_graph_node->output_pins[iport][ipin]),
        physical_pb_graph_node, vpr_device_annotation, verbose_output);
    }
    /* Finish a port, accumulate the port-level offset affiliated to the port */
    ...
    t_pb_graph_pin* operating_pb_graph_pin =
      &(operating_pb_graph_node->output_pins[iport][0]);
    for (t_port* candidate_port :
         vpr_device_annotation.physical_pb_port(operating_pb_graph_pin->port)) {
      vpr_device_annotation.accumulate_physical_pb_port_rotate_offset(
        operating_pb_graph_pin->port, candidate_port);
  ...
  for (int iport = 0; iport < operating_pb_graph_node->num_clock_ports;
       ++iport) {
    for (int ipin = 0; ipin < operating_pb_graph_node->num_clock_pins[iport];
         ++ipin) {
      annotate_physical_pb_graph_pin(
        &(operating_pb_graph_node->clock_pins[iport][ipin]),
        physical_pb_graph_node, vpr_device_annotation, verbose_output);
    }
    /* Finish a port, accumulate the port-level offset affiliated to the port */
    ...
    t_pb_graph_pin* operating_pb_graph_pin =
      &(operating_pb_graph_node->clock_pins[iport][0]);
    for (t_port* candidate_port :
         vpr_device_annotation.physical_pb_port(operating_pb_graph_pin->port)) {
      vpr_device_annotation.accumulate_physical_pb_port_rotate_offset(
        operating_pb_graph_pin->port, candidate_port);
 ...
 * Here, we will follow the strategy where primitive nodes are visited first
 *******************************************************************/
static void rec_build_vpr_physical_pb_graph_node_annotation(
  t_pb_graph_node* pb_graph_node, VprDeviceAnnotation& vpr_device_annotation,
  const bool& verbose_output) {
  /* Go recursive first until we touch the primitive node */
  if (false == is_primitive_pb_type(pb_graph_node->pb_type)) {
    for (int imode = 0; imode < pb_graph_node->pb_type->num_modes; ++imode) {
      for (int ipb = 0;
        ...
        for (int jpb = 0;
             ...
             pb_graph_node->pb_type->modes[imode].pb_type_children[ipb].num_pb;
             ++jpb) {
          rec_build_vpr_physical_pb_graph_node_annotation(
            &(pb_graph_node->child_pb_graph_nodes[imode][ipb][jpb]),
            vpr_device_annotation, verbose_output);
        }
      }
   ...
   *  - Get the physical pb_type that this type of pb_graph_node should be
   * mapped to
   ...
   * To bind pins from operating pb_graph_node to their physical pb_graph_node
   * pins
   */
  t_pb_type* physical_pb_type =
    vpr_device_annotation.physical_pb_type(pb_graph_node->pb_type);
  VTR_ASSERT(nullptr != physical_pb_type);

  /* Index inference:
   * physical_pb_graph_node_unique_index = operating_pb_graph_node_unique_index
   * * factor + offset where factor and offset are provided by users
   */
  PbGraphNodeId physical_pb_graph_node_id = PbGraphNodeId(
    vpr_device_annotation.physical_pb_type_index_factor(
      pb_graph_node->pb_type) *
      (size_t)vpr_device_annotation.pb_graph_node_unique_index(pb_graph_node) +
    vpr_device_annotation.physical_pb_type_index_offset(
      pb_graph_node->pb_type));

  t_pb_graph_node* physical_pb_graph_node = vpr_device_annotation.pb_graph_node(
    physical_pb_type, physical_pb_graph_node_id);
  VTR_ASSERT(nullptr != physical_pb_graph_node);
  vpr_device_annotation.add_physical_pb_graph_node(pb_graph_node,
  ...
  annotate_physical_pb_graph_node_pins(pb_graph_node, physical_pb_graph_node,
                                       vpr_device_annotation, verbose_output);
}
 ...
 *******************************************************************/
static void annotate_physical_pb_graph_node(
  const DeviceContext& vpr_device_ctx,
  VprDeviceAnnotation& vpr_device_annotation, const bool& verbose_output) {
  for (const t_logical_block_type& lb_type :
       vpr_device_ctx.logical_block_types) {
...
void annotate_pb_graph(const DeviceContext& vpr_device_ctx,
  ...
  VTR_LOGV(verbose_output, "\n");
  annotate_primitive_pb_graph_node_unique_index(vpr_device_ctx,
                                                vpr_device_annotation);
  VTR_LOG("Done\n");
  ...
  VTR_LOGV(verbose_output, "\n");
  annotate_physical_pb_graph_node(vpr_device_ctx, vpr_device_annotation,
                                  verbose_output);
  VTR_LOG("Done\n");

  /* Check each primitive pb_graph_node and pin has been binded to a physical
   * node and pin */
  check_physical_pb_graph_node_annotation(
    vpr_device_ctx,
