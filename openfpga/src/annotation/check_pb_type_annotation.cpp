 ...
 *******************************************************************/
static void rec_check_vpr_physical_pb_mode_annotation(
  t_pb_type* cur_pb_type, const bool& expect_physical_mode,
  const VprDeviceAnnotation& vpr_device_annotation, size_t& num_err) {
  ...
  if (true == expect_physical_mode) {
    ...
    if (nullptr != vpr_device_annotation.physical_mode(cur_pb_type)) {
      VTR_LOG_ERROR(
        "Find a physical mode '%s' for pb_type '%s' which is not under any "
        "physical mode!\n",
  ...
  for (int imode = 0; imode < cur_pb_type->num_modes; ++imode) {
    ...
    if (&(cur_pb_type->modes[imode]) ==
        vpr_device_annotation.physical_mode(cur_pb_type)) {
      expect_child_physical_mode = true && expect_physical_mode;
    }
    for (int ichild = 0;
         ichild < cur_pb_type->modes[imode].num_pb_type_children; ++ichild) {
      rec_check_vpr_physical_pb_mode_annotation(
        &(cur_pb_type->modes[imode].pb_type_children[ichild]),
        expect_child_physical_mode, vpr_device_annotation, num_err);
    }
  }
}
 ...
 *******************************************************************/
void check_vpr_physical_pb_mode_annotation(
  ...
  for (const t_logical_block_type& lb_type :
    ...
    rec_check_vpr_physical_pb_mode_annotation(lb_type.pb_type, true,
  ...
  if (0 == num_err) {
    ...
    VTR_LOG_ERROR(
      "Check physical mode annotation for pb_types failed with %ld errors!\n",
      num_err);
  }
}
 ...
 *******************************************************************/
static void check_vpr_physical_primitive_pb_type_annotation(
  ...
  if (nullptr == vpr_device_annotation.physical_pb_type(cur_pb_type)) {
    VTR_LOG_ERROR(
      "Find a pb_type '%s' which has not been mapped to any physical "
      "pb_type!\n",
      cur_pb_type->name);
    VTR_LOG_ERROR("Please specify in the OpenFPGA architecture\n");
    num_err++;
  ...
  for (t_port* pb_port : pb_type_ports(cur_pb_type)) {
    if (0 == vpr_device_annotation.physical_pb_port(pb_port).size()) {
      VTR_LOG_ERROR(
        "Find a port '%s' of pb_type '%s' which has not been mapped to any "
        "physical port!\n",
 ...
 * 2. physical mode appears only when its parent is a physical mode.
 *******************************************************************/
static void rec_check_vpr_physical_pb_type_annotation(
  t_pb_type* cur_pb_type, const VprDeviceAnnotation& vpr_device_annotation,
  size_t& num_err) {
  /* Primitive pb_type should always been binded to a physical pb_type */
  if (true == is_primitive_pb_type(cur_pb_type)) {
    check_vpr_physical_primitive_pb_type_annotation(
      cur_pb_type, vpr_device_annotation, num_err);
    return;
  }
  ...
  for (int imode = 0; imode < cur_pb_type->num_modes; ++imode) {
    for (int ichild = 0;
         ichild < cur_pb_type->modes[imode].num_pb_type_children; ++ichild) {
      rec_check_vpr_physical_pb_type_annotation(
        &(cur_pb_type->modes[imode].pb_type_children[ichild]),
        vpr_device_annotation, num_err);
    }
  }
 ...
 *******************************************************************/
void check_vpr_physical_pb_type_annotation(
  const DeviceContext& vpr_device_ctx,
  const VprDeviceAnnotation& vpr_device_annotation) {
  size_t num_err = 0;
  ...
  for (const t_logical_block_type& lb_type :
    ...
    /* Top pb_type should always has a physical mode! */
    rec_check_vpr_physical_pb_type_annotation(lb_type.pb_type,
                                              vpr_device_annotation, num_err);
  }
  if (0 == num_err) {
    ...
    VTR_LOG_ERROR(
      "Check physical pb_type annotation for pb_types failed with %ld "
      "errors!\n",
      num_err);
  }
}
 ...
 *  - Every port of the pb_type have been linked to a valid port of a circuit
 *model
 *  - Every interconnect has been linked to a valid circuit model
 *    in a correct type
 *******************************************************************/
static void rec_check_vpr_pb_type_circuit_model_annotation(
  ...
  if (true == is_primitive_pb_type(cur_pb_type)) {
    ...
    if (CircuitModelId::INVALID() ==
        vpr_device_annotation.pb_type_circuit_model(cur_pb_type)) {
      VTR_LOG_ERROR(
        "Found a physical pb_type '%s' missing circuit model binding!\n",
    ...
    for (t_port* port : pb_type_ports(cur_pb_type)) {
      if (CircuitPortId::INVALID() ==
          vpr_device_annotation.pb_circuit_port(port)) {
        VTR_LOG_ERROR(
          "Found a port '%s' of physical pb_type '%s' missing circuit port "
          "binding!\n",
          port->name, generate_pb_type_hierarchy_path(cur_pb_type).c_str());
        num_err++;
      }
    }
    return;
  ...
  /* Every interconnect in the physical mode has been linked to a valid circuit
   * model in a correct type */
  ...
  for (t_interconnect* interc : pb_mode_interconnects(physical_mode)) {
    CircuitModelId interc_circuit_model =
      vpr_device_annotation.interconnect_circuit_model(interc);
    ...
    if (circuit_lib.model_type(interc_circuit_model) !=
  ...
  for (int ichild = 0; ichild < physical_mode->num_pb_type_children; ++ichild) {
    rec_check_vpr_pb_type_circuit_model_annotation(
 ...
 *    in a correct type
 *******************************************************************/
void check_vpr_pb_type_circuit_model_annotation(
  ...
  for (const t_logical_block_type& lb_type :
    ...
    /* Top pb_type should always has a physical mode! */
    rec_check_vpr_pb_type_circuit_model_annotation(
      lb_type.pb_type, circuit_lib, vpr_device_annotation, num_err);
  }
  if (0 == num_err) {
    VTR_LOG("Check physical pb_type annotation for circuit model passed.\n");
  } else {
    VTR_LOG_ERROR(
      "Check physical pb_type annotation for circuit model failed with %ld "
      "errors!\n",
      num_err);
  }
}
 ...
 *  - If a primitive pb_type has mode bits, it must have been linked to a
 *physical pb_type and the circuit model must have a port for mode selection.
 *    And the port size must match the length of mode bits
 *******************************************************************/
static void rec_check_vpr_pb_type_mode_bits_annotation(
  t_pb_type* cur_pb_type, const CircuitLibrary& circuit_lib,
  const VprDeviceAnnotation& vpr_device_annotation, size_t& num_err) {
  /* Primitive pb_type should always been binded to a physical pb_type */
  if (true == is_primitive_pb_type(cur_pb_type)) {
    /* Find the physical pb_type
     * If the physical pb_type has mode selection bits, this pb_type must have
     * as well!
     */
    t_pb_type* physical_pb_type =
      vpr_device_annotation.physical_pb_type(cur_pb_type);
    ...
    if (vpr_device_annotation.pb_type_mode_bits(cur_pb_type).size() !=
        vpr_device_annotation.pb_type_mode_bits(physical_pb_type).size()) {
      VTR_LOG_ERROR(
        "Found different sizes of mode_bits for pb_type '%s' and its physical "
        "pb_type '%s'\n",
        cur_pb_type->name, physical_pb_type->name);
      num_err++;
      return;
    }

    /* Try to find a mode selection port for the circuit model linked to the
     * circuit model */
    CircuitModelId circuit_model =
      vpr_device_annotation.pb_type_circuit_model(physical_pb_type);
    if (CircuitModelId::INVALID() ==
        vpr_device_annotation.pb_type_circuit_model(physical_pb_type)) {
      VTR_LOG_ERROR(
        "Found a physical pb_type '%s' missing circuit model binding!\n",
    ...
    /* Search the ports of this circuit model and we must have a mode selection
     * port */
    std::vector<CircuitPortId> mode_select_ports =
      find_circuit_mode_select_sram_ports(circuit_lib, circuit_model);
    ...
    if (port_num_mode_bits !=
        vpr_device_annotation.pb_type_mode_bits(cur_pb_type).size()) {
      VTR_LOG_ERROR(
        "Length of mode bits of pb_type '%s' does not match the size(%ld) of "
        "mode selection ports of circuit model '%s'!\n",
        cur_pb_type->name, port_num_mode_bits,
  ...
  for (int imode = 0; imode < cur_pb_type->num_modes; ++imode) {
    for (int ichild = 0;
         ichild < cur_pb_type->modes[imode].num_pb_type_children; ++ichild) {
      rec_check_vpr_pb_type_mode_bits_annotation(
        &(cur_pb_type->modes[imode].pb_type_children[ichild]), circuit_lib,
        vpr_device_annotation, num_err);
    }
  }
}
 ...
 *    a port for mode selection. And the port size must match the length of mode
 *bits
 *
 * Note:
 ...
 *    is completed
 *******************************************************************/
void check_vpr_pb_type_mode_bits_annotation(
  const DeviceContext& vpr_device_ctx, const CircuitLibrary& circuit_lib,
  const VprDeviceAnnotation& vpr_device_annotation) {
  size_t num_err = 0;
  ...
  for (const t_logical_block_type& lb_type :
    ...
    /* Top pb_type should always has a physical mode! */
    rec_check_vpr_pb_type_mode_bits_annotation(lb_type.pb_type, circuit_lib,
                                               vpr_device_annotation, num_err);
  }
  if (0 == num_err) {
    ...
    VTR_LOG_ERROR(
      "Check physical pb_type annotation for mode selection bits failed with "
      "%ld errors!\n",
      num_err);
  }
}
