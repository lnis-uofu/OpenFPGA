 ...
 *******************************************************************/
static float average_atom_net_signal_density(
 ...
 *******************************************************************/
static float average_weighted_atom_net_signal_density(
  const AtomContext& atom_ctx,
  const std::unordered_map<AtomNetId, t_net_power>& net_activity) {
 ...
 *******************************************************************/
static size_t median_atom_net_signal_density(
  const AtomContext& atom_ctx,
  const std::unordered_map<AtomNetId, t_net_power>& net_activity) {
  ...
  return 0.5 * (net_densities[size_t((net_densities.size() - 1) / 2)] +
                net_densities[size_t((net_densities.size() - 1) / 2)]);
}

/********************************************************************
 * Find the number of clock cycles in simulation based on the average signal
 *density
 *******************************************************************/
static size_t recommend_num_sim_clock_cycle(
  const AtomContext& atom_ctx,
  const std::unordered_map<AtomNetId, t_net_power>& net_activity,
  const float& sim_window_size) {
  float average_density =
    average_atom_net_signal_density(atom_ctx, net_activity);
  float average_weighted_density =
    average_weighted_atom_net_signal_density(atom_ctx, net_activity);
  ...
  /* We have three choices in selecting the number of clock cycles based on
   * signal density
  ...
  if ((0. == median_density) && (0. == average_density)) {
    recmd_num_clock_cycles = 1;
    VTR_LOG_WARN(
      "All the signal density is zero!\nNumber of clock cycles in simulations "
      "are set to be %ld!\n",
    ...
    /* add a sim window size to balance the weight of average density and median
     * density In practice, we find that there could be huge difference between
     * avereage and median values For a reasonable number of simulation clock
     * cycles, we do this window size.
     */
    recmd_num_clock_cycles =
      (size_t)round(1 / (sim_window_size * average_density +
                         (1 - sim_window_size) * median_density));

    VTR_LOG("Window size set for simulation: %.2f\n", sim_window_size);
    VTR_LOG("Net density after applying window size : %.2f\n",
            (sim_window_size * average_density +
             (1 - sim_window_size) * median_density));
  }
 ...
 *  - If the number of clock cycles in simulation is set to be automatically
 *determined, we will infer the number based on the average signal density
 *******************************************************************/
int annotate_simulation_setting(
  ...
  if (0. == sim_setting.default_operating_clock_frequency()) {
    ...
    float T_crit = timing_info->least_slack_critical_path().delay() *
                   (1. + sim_setting.operating_clock_frequency_slack());
    sim_setting.set_default_operating_clock_frequency(1 / T_crit);
    VTR_LOG(
      "Use VPR critical path delay %g [ns] with a %g [%] slack in OpenFPGA.\n",
  ...
  for (const SimulationClockId& clock_id : sim_setting.clocks()) {
    if (0. == sim_setting.clock_frequency(clock_id)) {
      sim_setting.set_clock_frequency(
        clock_id, sim_setting.default_operating_clock_frequency());
    }
    VTR_LOG(
      "Will apply clock frequency %g [MHz] to clock '%s[%d:%d]' in "
      "simulations\n",
  ...
  if (0. == sim_setting.num_clock_cycles()) {
     ...
     */
    VTR_LOG(
      "User specified the number of operating clock cycles to be inferred from "
      "signal activities\n");

    /* Error out if net activity is not defined */
    if (true == net_activity.empty()) {
      VTR_LOG_ERROR(
        "Signal activities are not defined!\nPlease check if activity file is "
        "given and properly generated!\n");
      return CMD_EXEC_FATAL_ERROR;
    }

    /* Use a fixed simulation window size now. TODO: this could be specified by
     * users */
    size_t num_clock_cycles =
      recommend_num_sim_clock_cycle(atom_ctx, net_activity, 0.5);
  ...
  if (MIN_NUM_SIM_OPERATING_CLOCK_CYCLES > sim_setting.num_clock_cycles()) {
    VTR_LOG_ERROR(
      "Minimum number of operating clock cycles applicable to simulations is "
      "%d, while the specified number is %lu!\n",
      MIN_NUM_SIM_OPERATING_CLOCK_CYCLES, sim_setting.num_clock_cycles());
    return CMD_EXEC_FATAL_ERROR;
  }
