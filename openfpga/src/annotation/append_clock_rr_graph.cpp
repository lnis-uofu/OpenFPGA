...
static size_t estimate_clock_rr_graph_num_nodes(
  const DeviceGrid& grids, const size_t& layer, const int rr_graph_x_offset,
  const int rr_graph_y_offset, const bool& perimeter_cb,
  ...
  for (size_t iy = chanx_bb.ymin(); iy < chanx_bb.ymax(); ++iy) {
    for (size_t ix = chanx_bb.xmin(); ix < chanx_bb.xmax(); ++ix) {
      ...
      if ((false == through_channel) &&
          (false == is_chanx_exist(grids, layer, chanx_coord, rr_graph_x_offset,
                                   perimeter_cb))) {
        continue;
      }
  ...
  for (size_t ix = chany_bb.xmin(); ix < chany_bb.xmax(); ++ix) {
    for (size_t iy = chany_bb.ymin(); iy < chany_bb.ymax(); ++iy) {
       ...
       * not allowed */
      if ((false == through_channel) &&
          (false == is_chany_exist(grids, layer, chany_coord, rr_graph_y_offset,
                                   perimeter_cb))) {
        continue;
      }
      /* Estimate the routing tracks required by clock routing only */
...
static void add_rr_graph_clock_nodes(
  ...
  const size_t& layer, const int rr_graph_x_offset, const int rr_graph_y_offset,
...
int append_clock_rr_graph(DeviceContext& vpr_device_ctx,
  ...
  size_t num_clock_nodes = estimate_clock_rr_graph_num_nodes(
    vpr_device_ctx.grid, 0, rr_graph_x_offset, rr_graph_y_offset,
  ...
  add_rr_graph_clock_edges(
    ...
    vpr_device_ctx.rr_graph, vpr_device_ctx.grid, 0, rr_graph_x_offset,
    rr_graph_y_offset, vpr_device_ctx.arch->perimeter_cb,
