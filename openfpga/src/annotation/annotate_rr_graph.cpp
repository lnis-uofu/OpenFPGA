  ...
  for (size_t side = 0; side < rr_gsb.get_num_sides(); ++side) {
    ...
    switch (side) {
      case TOP: /* TOP = 0 */
        ...
        /* Side: TOP => 0, RIGHT => 1, BOTTOM => 2, LEFT => 3 */
        /* Create a rr_chan object and check if it is unique in the graph */
        rr_chan = build_one_rr_chan(vpr_device_ctx, e_rr_type::CHANY, layer,
                                    coordinate);
      ...
      case RIGHT: /* RIGHT = 1 */
        ...
        /* Create a rr_chan object and check if it is unique in the graph */
        rr_chan = build_one_rr_chan(vpr_device_ctx, e_rr_type::CHANX, layer,
                                    coordinate);
        chan_dir_to_port_dir_mapping[0] =
          OUT_PORT; /* INC_DIRECTION => OUT_PORT */
        chan_dir_to_port_dir_mapping[1] =
...
void annotate_device_rr_gsb(const DeviceContext& vpr_device_ctx,
  ...
  /* Must set version before reserve. Other RRGSB version is not passed into actual data */
  ...
  for (size_t ix = 0; ix < gsb_range.x(); ++ix) {
    for (size_t iy = 0; iy < gsb_range.y(); ++iy) {
      ...
      const RRGSB& rr_gsb = build_rr_gsb(
        ...
        vpr_device_ctx.arch->perimeter_cb, include_clock, in_edges, gsb_version);
