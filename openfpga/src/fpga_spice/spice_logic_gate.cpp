...
int print_spice_and_gate_subckt(std::fstream& fp,
  ...
  for (const auto& input_port : input_ports) {
    for (int ibin = 0; ibin < num_pmos_bins; ++ibin) {
      ...
      status = print_spice_generic_pmos_modeling(
        fp, std::to_string(ibin), std::string(SPICE_SUBCKT_VDD_PORT_NAME),
  ...
  for (size_t input_id = 0; input_id < input_ports.size(); ++input_id) {
    for (int ibin = 0; ibin < num_nmos_bins; ++ibin) {
      ...
      /* Depending on the input id, we assign different port names to
       * source/drain */
      std::string source_port_name;
      std::string drain_port_name;

      if (0 == input_id) {
        /* First transistor should connect to the output port and an internal
         * node */
        source_port_name = circuit_lib.port_prefix(output_ports[0]);
        drain_port_name =
          std::string("internal_node") + std::to_string(input_id);
      } else if (input_id == input_ports.size() - 1) {
        /* Last transistor should connect to an internal node and GND */
        source_port_name =
          std::string("internal_node") + std::to_string(input_id - 1);
      ...
      status = print_spice_generic_nmos_modeling(
        fp, std::to_string(ibin), source_port_name,
        circuit_lib.port_prefix(input_ports[input_id]), drain_port_name,
...
int print_spice_or_gate_subckt(std::fstream& fp,
  ...
  for (const auto& input_port : input_ports) {
    for (int ibin = 0; ibin < num_nmos_bins; ++ibin) {
      ...
      status = print_spice_generic_nmos_modeling(
        fp, std::to_string(ibin), circuit_lib.port_prefix(output_ports[0]),
        circuit_lib.port_prefix(input_port),
        std::string(SPICE_SUBCKT_GND_PORT_NAME), tech_lib, tech_model,
        curr_bin_width);
      if (CMD_EXEC_FATAL_ERROR == status) {
        return status;
      }
