 ...
 ***********************************************/
static void print_spice_supply_wrapper_subckt(
  ...
  for (const ModulePortId& module_port_id :
    ...
    for (const auto& pin : module_port.pins()) {
      ...
      print_spice_short_connection(fp, generate_spice_port(spice_pin, true),
                                   const_pin_name);
    }
  }
...
int print_spice_supply_wrappers(NetlistManager& netlist_manager,
  ...
  std::string spice_fname =
    submodule_dir + std::string(SUPPLY_WRAPPER_SPICE_FILE_NAME);
...
int print_spice_essential_gates(
  ...
  for (const CircuitModelId& circuit_model : circuit_lib.models()) {
    ...
    VTR_LOG("Generating SPICE netlist '%s' for circuit model '%s'...",
            spice_fname.c_str(), circuit_lib.model_name(circuit_model).c_str());
    ...
    if (CIRCUIT_MODEL_INVBUF == circuit_lib.model_type(circuit_model)) {
      if (CIRCUIT_MODEL_BUF_INV == circuit_lib.buffer_type(circuit_model)) {
        ...
        VTR_ASSERT(CIRCUIT_MODEL_BUF_BUF ==
                   circuit_lib.buffer_type(circuit_model));
        status =
          print_spice_buffer_subckt(fp, module_manager, module_id, circuit_lib,
                                    circuit_model, tech_lib, tech_model);
        netlist_filled = true;
      }
    ...
    if (CIRCUIT_MODEL_PASSGATE == circuit_lib.model_type(circuit_model)) {
      status =
        print_spice_passgate_subckt(fp, module_manager, module_id, circuit_lib,
                                    circuit_model, tech_lib, tech_model);
      netlist_filled = true;
    ...
    if (CIRCUIT_MODEL_GATE == circuit_lib.model_type(circuit_model)) {
      if (CIRCUIT_MODEL_GATE_AND == circuit_lib.gate_type(circuit_model)) {
        status = print_spice_and_gate_subckt(fp, module_manager, module_id,
                                             circuit_lib, circuit_model,
                                             tech_lib, tech_model);
        netlist_filled = true;
      } else if (CIRCUIT_MODEL_GATE_OR ==
                 circuit_lib.gate_type(circuit_model)) {
        status =
          print_spice_or_gate_subckt(fp, module_manager, module_id, circuit_lib,
                                     circuit_model, tech_lib, tech_model);
        netlist_filled = true;
      }
    ...
    if (false == netlist_filled) {
      VTR_LOG_ERROR(
        "Cannot auto-generate netlist for circuit model '%s'!\n\tThe circuit "
    ...
    netlist_manager.set_netlist_type(nlist_id,
                                     NetlistManager::SUBMODULE_NETLIST);
