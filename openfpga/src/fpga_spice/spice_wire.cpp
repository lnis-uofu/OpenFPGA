 ...
 *      GND     GND    GND           GND
 *******************************************************************/
static int print_spice_wire_pi_type_rc_modeling(
  std::fstream& fp, const std::string& input_port_name,
  const std::string& output_port_name,
  const std::string& middle_output_port_name, const float& res_total,
  const float& cap_total, const size_t& num_levels) {
  ...
  /* All the resistance and capacitance value should be larger than or equal to
   * zero*/
  ...
  for (size_t ilvl = 0; ilvl < num_levels; ++ilvl) {
    ...
    if ((0 == ilvl) && (0. < cap_per_level)) {
      print_spice_capacitor(fp, input_port_name,
    ...
    std::string lvl_input_port_name =
      std::string("rc_network_node") + std::to_string(ilvl);
    if (0 == ilvl) {
      lvl_input_port_name = input_port_name;
    }

    std::string lvl_middle_port_name =
      std::string("rc_network_midnode") + std::to_string(ilvl);

    std::string lvl_output_port_name =
      std::string("rc_network_node") + std::to_string(ilvl + 1);
    ...
    print_spice_resistor(fp, lvl_input_port_name, lvl_middle_port_name,
                         res_per_level);
    print_spice_resistor(fp, lvl_middle_port_name, lvl_output_port_name,
                         res_per_level);
    ...
    if (0. < cap_curr_level) {
      print_spice_capacitor(fp, lvl_output_port_name,
                            std::string(SPICE_SUBCKT_GND_PORT_NAME),
                            cap_curr_level);
    }
  }
  ...
  if (!middle_output_port_name.empty()) {
    ...
    std::string rc_midnode_name =
      std::string("rc_network_node") + std::to_string(num_levels / 2 + 1);
    if (1 == num_levels % 2) {
      rc_midnode_name =
        std::string("rc_network_midnode") + std::to_string(num_levels / 2);
    }
 ...
 *           GND    GND    GND                GND
 *******************************************************************/
static int print_spice_wire_t_type_rc_modeling(
  std::fstream& fp, const std::string& input_port_name,
  const std::string& output_port_name,
  const std::string& middle_output_port_name, const float& res_total,
  const float& cap_total, const size_t& num_levels) {
  ...
  for (size_t ilvl = 0; ilvl < num_levels; ++ilvl) {
    ...
    if (0. < cap_per_level) {
      print_spice_capacitor(fp, lvl_middle_port_name,
                            std::string(SPICE_SUBCKT_GND_PORT_NAME),
                            cap_per_level);
    }
  }
 ...
 *                                      |
 *      +--------------------+    +---------------+     +--------------------+
 * in ->| Inverter or buffer |--->|   RC Network  |---->| Inverter or buffer
...
int print_spice_wire_subckt(std::fstream& fp,
  ...
  std::string rc_ntwk_input_port_name =
    std::string("rc_network_node") + std::to_string(0);
  std::string rc_ntwk_output_port_name =
    std::string("rc_network_node") +
    std::to_string(circuit_lib.wire_num_level(circuit_model) - 1);
  std::string rc_ntwk_middle_output_port_name;
  if (CIRCUIT_MODEL_CHAN_WIRE == circuit_lib.model_type(circuit_model)) {
    rc_ntwk_middle_output_port_name =
      std::string("middle") + rc_ntwk_output_port_name;
  }

  ModulePortId wire_module_input_port =
    module_manager.find_module_port(module_id, input_port_name);
  ModulePortId wire_module_output_port =
    module_manager.find_module_port(module_id, output_port_name);
  ModulePortId wire_module_middle_output_port = ModulePortId::INVALID();
  if (CIRCUIT_MODEL_CHAN_WIRE == circuit_lib.model_type(circuit_model)) {
    wire_module_middle_output_port =
      module_manager.find_module_port(module_id, output_port_name);
  }
  ...
  if (circuit_lib.input_buffer_model(circuit_model)) {
    ...
    ModuleId buffer_module = module_manager.find_module(
      circuit_lib.model_name(circuit_lib.input_buffer_model(circuit_model)));
    VTR_ASSERT(true == module_manager.valid_module_id(buffer_module));

    ModulePortId module_input_port_id = find_inverter_buffer_module_port(
      module_manager, buffer_module, circuit_lib, circuit_model,
      CIRCUIT_MODEL_PORT_INPUT);
    ModulePortId module_output_port_id = find_inverter_buffer_module_port(
      module_manager, buffer_module, circuit_lib, circuit_model,
      CIRCUIT_MODEL_PORT_OUTPUT);

    /* Port size should be 1 ! */
    VTR_ASSERT(1 ==
               module_manager.module_port(buffer_module, module_input_port_id)
                 .get_width());
    VTR_ASSERT(1 ==
               module_manager.module_port(buffer_module, module_output_port_id)
                 .get_width());

    port2port_name_map[module_manager
                         .module_port(buffer_module, module_input_port_id)
                         .get_name()] =
      module_manager.module_port(module_id, wire_module_input_port);
    port2port_name_map[module_manager
                         .module_port(buffer_module, module_output_port_id)
                         .get_name()] = BasicPort(rc_ntwk_input_port_name, 1);

    print_spice_subckt_instance(fp, module_manager, buffer_module,
                                instance_name, port2port_name_map);
  } else {
    print_spice_short_connection(fp, circuit_lib.port_prefix(input_ports[0]),
  ...
  switch (circuit_lib.wire_type(circuit_model)) {
    case WIRE_MODEL_PI:
      status = print_spice_wire_pi_type_rc_modeling(
        fp, rc_ntwk_input_port_name, rc_ntwk_output_port_name,
        rc_ntwk_middle_output_port_name, circuit_lib.wire_r(circuit_model),
        circuit_lib.wire_c(circuit_model),
        circuit_lib.wire_num_level(circuit_model));
      break;
    case WIRE_MODEL_T:
      status = print_spice_wire_t_type_rc_modeling(
        fp, rc_ntwk_input_port_name, rc_ntwk_output_port_name,
        rc_ntwk_middle_output_port_name, circuit_lib.wire_r(circuit_model),
        circuit_lib.wire_c(circuit_model),
        circuit_lib.wire_num_level(circuit_model));
  ...
  if (circuit_lib.output_buffer_model(circuit_model)) {
    ...
    ModuleId buffer_module = module_manager.find_module(
      circuit_lib.model_name(circuit_lib.output_buffer_model(circuit_model)));
    VTR_ASSERT(true == module_manager.valid_module_id(buffer_module));
    ...
    port2port_name_map[module_manager
                         .module_port(buffer_module, module_input_port_id)
                         .get_name()] = BasicPort(rc_ntwk_output_port_name, 1);
    port2port_name_map[module_manager
                         .module_port(buffer_module, module_output_port_id)
                         .get_name()] =
      module_manager.module_port(module_id, wire_module_output_port);
    ...
    if (!rc_ntwk_middle_output_port_name.empty()) {
      ...
      port2port_name_map[module_manager
                           .module_port(buffer_module, module_output_port_id)
                           .get_name()] =
        module_manager.module_port(module_id, wire_module_middle_output_port);

      print_spice_subckt_instance(fp, module_manager, buffer_module,
                                  instance_name, port2port_name_map);
    }
  } else {
    print_spice_short_connection(fp, rc_ntwk_output_port_name,
                                 circuit_lib.port_prefix(output_ports[0]));
    if (!rc_ntwk_middle_output_port_name.empty()) {
      print_spice_short_connection(fp, rc_ntwk_middle_output_port_name,
                                   circuit_lib.port_prefix(output_ports[0]));
    }
  }
