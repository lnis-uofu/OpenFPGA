 ...
 *
 *******************************************************************/
static int print_spice_pass_transistor_subckt(
  ...
  float total_nmos_width =
    circuit_lib.pass_gate_logic_nmos_size(circuit_model) *
  ...
  for (int ibin = 0; ibin < num_nmos_bins; ++ibin) {
    ...
    status = print_spice_generic_nmos_modeling(
 ...
 *
 *******************************************************************/
static int print_spice_transmission_gate_subckt(
  std::fstream& fp, const ModuleManager& module_manager,
  const ModuleId& module_id, const CircuitLibrary& circuit_lib,
  const CircuitModelId& circuit_model, const TechnologyLibrary& tech_lib,
  const TechnologyModelId& tech_model) {
  ...
  float total_pmos_width =
    circuit_lib.pass_gate_logic_pmos_size(circuit_model) *
  ...
  for (int ibin = 0; ibin < num_pmos_bins; ++ibin) {
    ...
    status = print_spice_generic_pmos_modeling(
      fp, std::to_string(ibin), circuit_lib.port_prefix(input_ports[0]),
...
int print_spice_passgate_subckt(std::fstream& fp,
  ...
  if (CIRCUIT_MODEL_PASS_GATE_TRANSISTOR ==
      circuit_lib.pass_gate_logic_type(circuit_model)) {
    status = print_spice_pass_transistor_subckt(fp, module_manager, module_id,
                                                circuit_lib, circuit_model,
                                                tech_lib, tech_model);
  } else if (CIRCUIT_MODEL_PASS_GATE_TRANSMISSION ==
             circuit_lib.pass_gate_logic_type(circuit_model)) {
    status = print_spice_transmission_gate_subckt(fp, module_manager, module_id,
                                                  circuit_lib, circuit_model,
                                                  tech_lib, tech_model);
  }
