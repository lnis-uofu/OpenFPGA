...
ShellCommandId add_pack_command(openfpga::Shell<OpenfpgaContext>& shell,
                                ShellCommandClassId vpr_cmd_class) {
  ...
  return shell_cmd_pack_id;
}

void add_report_cluster_command(
  openfpga::Shell<OpenfpgaContext>& shell, ShellCommandClassId vpr_cmd_class,
  const std::vector<ShellCommandId>& dependent_cmds) {
  Command shell_cmd_report_cluster("report_cluster");
  CommandOptionId opt_report_cluster_query =
    shell_cmd_report_cluster.add_option(
      "query", false, "Filter clusters using a shell-style pattern");
  shell_cmd_report_cluster.set_option_short_name(opt_report_cluster_query, "q");
  shell_cmd_report_cluster.set_option_require_value(opt_report_cluster_query,
                                                    openfpga::OPT_STRING);
  ShellCommandId shell_cmd_report_cluster_id =
    shell.add_command(shell_cmd_report_cluster,
                      "Report the number of clusters generated by packing");
  shell.set_command_class(shell_cmd_report_cluster_id, vpr_cmd_class);
  shell.set_command_execute_function(shell_cmd_report_cluster_id,
                                     vpr::report_cluster_template);
  shell.set_command_dependency(shell_cmd_report_cluster_id, dependent_cmds);
}
...
void add_vpr_commands(openfpga::Shell<OpenfpgaContext>& shell) {
  ...
  add_report_cluster_command(shell, vpr_cmd_class, report_cluster_dependencies);
