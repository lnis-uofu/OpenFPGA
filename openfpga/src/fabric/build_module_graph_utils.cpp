...
ModulePortId find_inverter_buffer_module_port(
  ...
  std::vector<CircuitPortId> model_ports =
    circuit_lib.model_ports_by_type(model_id, port_type, true);
  VTR_ASSERT(1 == model_ports.size());

  /* Find the input and output module ports */
  ModulePortId module_port_id = module_manager.find_module_port(
    module_id, circuit_lib.port_prefix(model_ports[0]));
  VTR_ASSERT(true ==
 ...
 * This function will return the wire created for the output port of
 *inverter/buffer
 *
 *  parent_module
 *  +-----------------------------------------------------------------
...
ModuleNetId add_inverter_buffer_child_module_and_nets(
  ...
  ModulePortId module_input_port_id =
    find_inverter_buffer_module_port(module_manager, child_module, circuit_lib,
                                     model_id, CIRCUIT_MODEL_PORT_INPUT);
  ModulePortId module_output_port_id =
    find_inverter_buffer_module_port(module_manager, child_module, circuit_lib,
                                     model_id, CIRCUIT_MODEL_PORT_OUTPUT);

  /* Port size should be 1 ! */
  VTR_ASSERT(
    1 ==
    module_manager.module_port(child_module, module_input_port_id).get_width());
  VTR_ASSERT(1 ==
             module_manager.module_port(child_module, module_output_port_id)
               .get_width());
  ...
  module_manager.add_module_net_sink(parent_module, input_net, child_module,
                                     child_instance, module_input_port_id, 0);
  ...
  module_manager.add_module_net_source(parent_module, output_net, child_module,
                                       child_instance, module_output_port_id,
                                       0);
