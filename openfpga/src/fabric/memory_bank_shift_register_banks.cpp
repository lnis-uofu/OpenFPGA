...
std::vector<size_t> MemoryBankShiftRegisterBanks::bl_bank_unique_sizes() const {
  ...
  for (const auto& region : bl_bank_data_ports_) {
    for (const auto& bank : region) {
      ConfigRegionId region_id =
        ConfigRegionId(&region - &bl_bank_data_ports_[ConfigRegionId(0)]);
      FabricBitLineBankId bank_id =
        FabricBitLineBankId(&bank - &region[FabricBitLineBankId(0)]);
      size_t cur_bank_size = bl_bank_size(region_id, bank_id);
      if (unique_sizes.end() ==
          std::find(unique_sizes.begin(), unique_sizes.end(), cur_bank_size)) {
...
std::vector<ModuleId> MemoryBankShiftRegisterBanks::bl_bank_unique_modules()
  const {
  std::vector<ModuleId> unique_modules;
  for (const auto& region : bl_bank_modules_) {
    for (const auto& bank : region) {
      if (unique_modules.end() ==
          std::find(unique_modules.begin(), unique_modules.end(), bank)) {
...
size_t MemoryBankShiftRegisterBanks::bl_bank_size(
...
FabricKey::fabric_bit_line_bank_range MemoryBankShiftRegisterBanks::bl_banks(
...
std::vector<size_t> MemoryBankShiftRegisterBanks::wl_bank_unique_sizes() const {
  ...
  for (const auto& region : wl_bank_data_ports_) {
    for (const auto& bank : region) {
      ConfigRegionId region_id =
        ConfigRegionId(&region - &wl_bank_data_ports_[ConfigRegionId(0)]);
      FabricWordLineBankId bank_id =
        FabricWordLineBankId(&bank - &region[FabricWordLineBankId(0)]);
...
std::vector<ModuleId> MemoryBankShiftRegisterBanks::wl_bank_unique_modules()
  const {
  std::vector<ModuleId> unique_modules;
...
size_t MemoryBankShiftRegisterBanks::wl_bank_size(
...
FabricKey::fabric_word_line_bank_range MemoryBankShiftRegisterBanks::wl_banks(
  const ConfigRegionId& region_id) const {
  VTR_ASSERT(valid_region_id(region_id));
...
std::vector<BasicPort> MemoryBankShiftRegisterBanks::bl_bank_data_ports(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id) const {
  VTR_ASSERT(valid_bl_bank_id(region_id, bank_id));
  return bl_bank_data_ports_[region_id][bank_id];
}

std::vector<BasicPort> MemoryBankShiftRegisterBanks::wl_bank_data_ports(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id) const {
...
FabricBitLineBankId
MemoryBankShiftRegisterBanks::find_bl_shift_register_bank_id(
...
BasicPort MemoryBankShiftRegisterBanks::find_bl_shift_register_bank_data_port(
  const ConfigRegionId& region, const BasicPort& bl_port) const {
  if (is_bl_bank_dirty_) {
    build_bl_port_fast_lookup();
  }
...
std::vector<size_t>
MemoryBankShiftRegisterBanks::bl_shift_register_bank_sink_child_ids(
...
std::vector<size_t>
MemoryBankShiftRegisterBanks::bl_shift_register_bank_sink_child_pin_ids(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id,
  const BasicPort& src_port) const {
  VTR_ASSERT(valid_bl_bank_id(region_id, bank_id));
...
MemoryBankShiftRegisterBanks::bl_shift_register_bank_source_ports(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id) const {
  VTR_ASSERT(valid_bl_bank_id(region_id, bank_id));
  ...
  for (const BasicPort& wide_port : bl_bank_data_ports(region_id, bank_id)) {
    for (size_t ipin = 0; ipin < wide_port.pins().size(); ++ipin) {
      src_ports.push_back(
        BasicPort(std::string(MEMORY_BL_PORT_NAME), cur_pin, cur_pin));
      cur_pin++;
    }
  }
...
ModuleId MemoryBankShiftRegisterBanks::bl_shift_register_bank_module(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id) const {
  VTR_ASSERT(valid_bl_bank_id(region_id, bank_id));
  return bl_bank_modules_[region_id][bank_id];
}

size_t MemoryBankShiftRegisterBanks::bl_shift_register_bank_instance(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id) const {
  VTR_ASSERT(valid_bl_bank_id(region_id, bank_id));
  return bl_bank_instances_[region_id][bank_id];
}

FabricWordLineBankId
MemoryBankShiftRegisterBanks::find_wl_shift_register_bank_id(
...
BasicPort MemoryBankShiftRegisterBanks::find_wl_shift_register_bank_data_port(
  const ConfigRegionId& region, const BasicPort& wl_port) const {
  if (is_wl_bank_dirty_) {
    build_wl_port_fast_lookup();
  }
...
void MemoryBankShiftRegisterBanks::link_bl_shift_register_bank_to_module(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id,
...
void MemoryBankShiftRegisterBanks::link_bl_shift_register_bank_to_instance(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id,
...
void MemoryBankShiftRegisterBanks::link_wl_shift_register_bank_to_module(
...
void MemoryBankShiftRegisterBanks::link_wl_shift_register_bank_to_instance(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id,
  const size_t& instance_id) {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
...
void MemoryBankShiftRegisterBanks::add_bl_shift_register_bank_sink_node(
  const ConfigRegionId& region, const FabricBitLineBankId& bank,
  const BasicPort& src_port, const size_t& sink_child_id,
  ...
  bl_bank_sink_child_pin_ids_[region][bank][src_port].push_back(
    sink_child_pin_id);
}

void MemoryBankShiftRegisterBanks::add_wl_shift_register_bank_sink_node(
  const ConfigRegionId& region, const FabricWordLineBankId& bank,
  const BasicPort& src_port, const size_t& sink_child_id,
  const size_t& sink_child_pin_id) {
  VTR_ASSERT(valid_wl_bank_id(region, bank));
  wl_bank_sink_child_ids_[region][bank][src_port].push_back(sink_child_id);
  wl_bank_sink_child_pin_ids_[region][bank][src_port].push_back(
    sink_child_pin_id);
}

void MemoryBankShiftRegisterBanks::reserve_bl_shift_register_banks(
  const ConfigRegionId& region_id, const size_t& num_banks) {
  VTR_ASSERT(valid_region_id(region_id));
...
void MemoryBankShiftRegisterBanks::reserve_bl_shift_register_banks(
  const FabricRegionId& region_id, const size_t& num_banks) {
...
void MemoryBankShiftRegisterBanks::reserve_wl_shift_register_banks(
  const ConfigRegionId& region_id, const size_t& num_banks) {
  VTR_ASSERT(valid_region_id(region_id));
...
FabricBitLineBankId MemoryBankShiftRegisterBanks::create_bl_shift_register_bank(
...
void MemoryBankShiftRegisterBanks::add_data_port_to_bl_shift_register_bank(
...
FabricWordLineBankId
MemoryBankShiftRegisterBanks::create_wl_shift_register_bank(
  const FabricRegionId& region_id) {
  ConfigRegionId config_region_id = ConfigRegionId(size_t(region_id));
...
void MemoryBankShiftRegisterBanks::add_data_port_to_wl_shift_register_bank(
...
std::vector<size_t>
MemoryBankShiftRegisterBanks::wl_shift_register_bank_sink_child_ids(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id,
  const BasicPort& src_port) const {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
...
ModuleId MemoryBankShiftRegisterBanks::wl_shift_register_bank_module(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id) const {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
  return wl_bank_modules_[region_id][bank_id];
}

size_t MemoryBankShiftRegisterBanks::wl_shift_register_bank_instance(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id) const {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
...
std::vector<size_t>
MemoryBankShiftRegisterBanks::wl_shift_register_bank_sink_child_pin_ids(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id,
  const BasicPort& src_port) const {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
...
std::vector<BasicPort>
MemoryBankShiftRegisterBanks::wl_shift_register_bank_source_ports(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id) const {
  VTR_ASSERT(valid_wl_bank_id(region_id, bank_id));
  std::vector<BasicPort> src_ports;
  ...
  for (const BasicPort& wide_port : wl_bank_data_ports(region_id, bank_id)) {
    for (size_t ipin = 0; ipin < wide_port.pins().size(); ++ipin) {
      src_ports.push_back(
        BasicPort(std::string(MEMORY_WL_PORT_NAME), cur_pin, cur_pin));
      cur_pin++;
    }
  }
...
bool MemoryBankShiftRegisterBanks::valid_region_id(
  const ConfigRegionId& region) const {
  return size_t(region) < bl_bank_ids_.size();
}

bool MemoryBankShiftRegisterBanks::valid_bl_bank_id(
  const ConfigRegionId& region_id, const FabricBitLineBankId& bank_id) const {
...
bool MemoryBankShiftRegisterBanks::valid_wl_bank_id(
  const ConfigRegionId& region_id, const FabricWordLineBankId& bank_id) const {
  if (!valid_region_id(region_id)) {
    return false;
  }
    ...
    for (const auto& bank : region) {
      ...
      for (const auto& port : bank) {
        for (const size_t& bl_index : port.pins()) {
          BasicPort bl_port(std::string(MEMORY_BL_PORT_NAME), bl_index,
                            bl_index);
          BasicPort sr_bl_port(std::string(MEMORY_BL_PORT_NAME), cur_pin,
                               cur_pin);
          ConfigRegionId region_id =
            ConfigRegionId(&region - &bl_bank_data_ports_[ConfigRegionId(0)]);
          FabricBitLineBankId bank_id =
    ...
    for (const auto& bank : region) {
      ...
      for (const auto& port : bank) {
        for (const size_t& wl_index : port.pins()) {
          BasicPort wl_port(std::string(MEMORY_WL_PORT_NAME), wl_index,
                            wl_index);
          BasicPort sr_wl_port(std::string(MEMORY_WL_PORT_NAME), cur_pin,
                               cur_pin);
          ConfigRegionId region_id =
            ConfigRegionId(&region - &wl_bank_data_ports_[ConfigRegionId(0)]);
          FabricWordLineBankId bank_id =
