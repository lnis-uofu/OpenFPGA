 ...
 * The other is called pinA_lower which is located close to the bottom side of
 *this grid
 *
 * Similarly, we duplicate pins at TOP, RIGHT, BOTTOM and LEFT sides.
 * For LEFT side, upper and lower pins carry the indication in physical location
 *as RIGHT side. For TOP and BOTTOM side, upper pin is located close to the left
 *side of a grid, while lower pin is located close to the right side of a grid
 *
...
static void add_grid_module_net_connect_duplicated_pb_graph_pin(
  ...
  for (const e_side& side : grid_pin_sides) {
    ...
    if (0. == find_physical_tile_pin_Fc(grid_type_descriptor, grid_pin_index)) {
      ...
      /* Grid port always has only 1 pin, it is assumed when adding these ports
       * to the module if you need a change, please also change the port adding
       * codes
       */
       ...
       * For output-to-output connection, net_source is pb_graph_pin, while
       * net_sink is grid pin
       */
      module_manager.add_module_net_source(grid_module, net, child_module,
    ...
    std::string grid_upper_port_name = generate_grid_duplicated_port_name(
      pin_width, pin_height, subtile_index, side, pin_info, true);
    ModulePortId grid_module_upper_port_id =
      module_manager.find_module_port(grid_module, grid_upper_port_name);
    VTR_ASSERT(true == module_manager.valid_module_port_id(
                         grid_module, grid_module_upper_port_id));

    /* Find the lower port in grid_module */
    std::string grid_lower_port_name = generate_grid_duplicated_port_name(
      pin_width, pin_height, subtile_index, side, pin_info, false);
    ModulePortId grid_module_lower_port_id =
      module_manager.find_module_port(grid_module, grid_lower_port_name);
    VTR_ASSERT(true == module_manager.valid_module_port_id(
                         grid_module, grid_module_lower_port_id));
    ...
    module_manager.add_module_net_source(grid_module, net, child_module,
                                         ...
                                         child_module_pin_id);
    module_manager.add_module_net_sink(grid_module, net, grid_module, 0,
                                       grid_module_upper_port_id,
                                       grid_module_pin_id);
    module_manager.add_module_net_sink(grid_module, net, grid_module, 0,
                                       grid_module_lower_port_id,
                                       grid_module_pin_id);
  }
}
