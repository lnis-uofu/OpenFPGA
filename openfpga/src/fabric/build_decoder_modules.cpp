/***************************************************************************************
 * This file includes functions that are used to build modules for decoders,
 *including:
...
ModuleId build_frame_memory_decoder_module(ModuleManager& module_manager,
  ...
  module_manager.add_port(module_id, data_port,
  ...
  if (true == decoder_lib.use_data_inv_port(decoder)) {
    BasicPort data_inv_port(std::string(DECODER_DATA_OUT_INV_PORT_NAME),
                            data_size);
    module_manager.add_port(module_id, data_inv_port,
                            ModuleManager::MODULE_OUTPUT_PORT);
  }
...
ModuleId build_wl_memory_decoder_module(ModuleManager& module_manager,
  ...
  if (true == decoder_lib.use_readback(decoder)) {
    BasicPort data_ren_port(std::string(DECODER_DATA_READ_ENABLE_PORT_NAME),
                            data_size);
    module_manager.add_port(module_id, data_ren_port,
                            ModuleManager::MODULE_OUTPUT_PORT);
    module_manager.set_port_is_register(module_id, data_ren_port.get_name(),
                                        true);
  }
 ...
 *encoded. Therefore, the number of inputs is ceil(log(num_of_outputs)/log(2))
 ***************************************************************************************/
static void build_mux_local_decoder_module(ModuleManager& module_manager,
                                           const DecoderLibrary& decoder_lib,
                                           const DecoderId& decoder) {
  /* Get the number of inputs */
  size_t addr_size = decoder_lib.addr_size(decoder);
