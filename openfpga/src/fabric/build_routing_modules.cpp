...
void build_switch_block_module_short_interc(ModuleManager& module_manager, 
  ...
  case OPIN: { 
    rr_gsb.get_node_side_and_index(rr_graph, drive_rr_node, IN_PORT, input_pin_side, index);
    break;
  }
  case CHANX:
  case CHANY: {
...
void build_switch_block_mux_module(ModuleManager& module_manager, 
  ...
  CircuitModelId mux_model = device_annotation.rr_switch_circuit_model(switch_index);
  ...
  std::vector<CircuitPortId> mux_model_output_ports = circuit_lib.model_ports_by_type(mux_model, CIRCUIT_MODEL_PORT_OUTPUT, true);
