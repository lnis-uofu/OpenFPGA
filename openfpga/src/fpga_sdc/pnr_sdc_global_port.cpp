 ...
 *******************************************************************/
static void print_pnr_sdc_clock_port(std::fstream& fp,
 ...
 *******************************************************************/
static void print_pnr_sdc_global_clock_ports(
  std::fstream& fp, const float& time_unit, const ModuleManager& module_manager,
  ...
  for (const FabricGlobalPortId& global_port :
    ...
    BasicPort clock_port = module_manager.module_port(
      top_module, fabric_global_port_info.global_module_port(global_port));
 ...
 *******************************************************************/
static void print_pnr_sdc_global_non_clock_ports(
  std::fstream& fp, const float& time_unit,
  ...
  for (const FabricGlobalPortId& global_port :
    ...
    /* Reach here, it means a non-clock global port and we need print
     * constraints */
    float clock_period = operating_critical_path_delay;
    BasicPort non_clock_port = module_manager.module_port(
      top_module, fabric_global_port_info.global_module_port(global_port));
...
void print_pnr_sdc_global_ports(const PnrSdcOption& options,
  ...
  std::string timer_message =
    std::string("Write SDC for constraining clocks for P&R flow '") +
  ...
  print_pnr_sdc_global_clock_ports(fp, time_unit, module_manager, top_module,
                                   global_ports, sim_setting);

  if (true == constrain_non_clock_port) {
    print_pnr_sdc_global_non_clock_ports(
      fp, time_unit, 1. / sim_setting.default_operating_clock_frequency(),
